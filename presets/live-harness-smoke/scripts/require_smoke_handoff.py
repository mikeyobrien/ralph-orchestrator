...
def main() -> None:
    ...
    # Ralph roots engine state beneath `.ralph/autoloop`; a bare autoloop run
    # would use its own `.autoloop` default instead. This gate runs inside a
    # Ralph-launched loop, so it reads the Ralph-owned layout.
    engine_state_dir = project_dir / ".ralph" / "autoloop"
    journal = engine_state_dir / "journal.jsonl"
    state_dir = engine_state_dir / "runs" / run_id

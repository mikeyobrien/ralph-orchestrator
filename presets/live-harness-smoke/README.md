...
<workspace>/.ralph/autoloop/journal.jsonl
<workspace>/.ralph/autoloop/runs/<run-id>/smoke-evidence.txt
```

Ralph launches the engine with its runtime state rooted beneath
`.ralph/autoloop`, not autoloop's standalone `.autoloop` default, so the smoke
reads the run back from the Ralph-owned root.
...
`.ralph/autoloop/` state. The committed sanitized dogfood result is recorded

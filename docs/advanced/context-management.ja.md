...
コンテキストウィンドウを効果的に管理することは、Ralph Orchestrator の成功に極めて重要です。
AI エージェントのコンテキストウィンドウは限られており、それを超えると失敗やパフォーマンスの
低下を招くことがあります。
...
| エージェント | コンテキストウィンドウ | トークン上限 | おおよその文字数 |
|-------|---------------|-------------|----------------------|
| Claude | 200K トークン | 200,000 | 約 800,000 文字 |
| Gemini | 32K トークン | 32,768 | 約 130,000 文字 |
| Q Chat | 8K トークン | 8,192 | 約 32,000 文字 |

## コンテキストの構成要素

### 何がコンテキストを消費するか

1. **PROMPT.md ファイル** - タスクの説明
2. **以前の出力** - エージェントの応答
3. **ファイルの内容** - 変更されているコード
4. **システムメッセージ** - エージェントへの指示
5. **エラーメッセージ** - デバッグ情報
...
| 症状 | 考えられる原因 | 解決策 |
|---------|-------------|----------|
| エージェントが前の指示を忘れる | コンテキストウィンドウが満杯 | チェックポイントを作成してリセットする |
| 不完全な応答 | トークン上限に達している | プロンプトのサイズを減らす |
| 作業の繰り返し | コンテキストの喪失 | 要約を使う |
| 情報が欠けているというエラー | コンテキストが切り詰められた | より小さなタスクに分割する |

# Ralph で CLI ツールを構築する

この例は、argparse、サブコマンド、適切なパッケージングを備えたコマンドラインツールを、
Ralph Orchestrator を使って作成する方法を示します。

## タスクの説明

次を備えたファイル管理用の Python CLI ツールを作成します。
- 複数のサブコマンド
- プログレスバー
- 設定ファイルのサポート
- エラー処理
...
1. **明確なコマンド構造**: すべてのコマンドとオプションを最初に定義する
2. **ユーザー体験**: 色付き出力とプログレスバーを求める
3. **エラー処理**: エラーをどう表示すべきかを指定する
4. **設定**: 最初から設定ファイルのサポートを含める
5. **テスト**: 各コマンドのユニットテストを求める
...
- **イテレーション**: 完全な実装で約 30〜40
- **時間**: 約 15〜20 分
- **エージェント**: Claude または Gemini
- **API 呼び出し**: 約 $0.30〜0.40

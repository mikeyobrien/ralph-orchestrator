# Ralph でデータ分析スクリプトを作る

この例は、pandas、可視化、レポート作成を伴うデータ分析スクリプトを、Ralph Orchestrator
を使って作成する方法を示します。

## タスクの説明

次のような Python データ分析スクリプトを作成します。
- CSV データを読み込みクレンジングする
- 統計分析を行う
- 可視化を作成する
- HTML レポートを生成する
...
1. **データ構造を指定する**: 入力データの形式を明確に定義する
2. **必要な分析を列挙する**: 必要な計算について具体的にする
3. **可視化を求める**: チャートの種類とライブラリを指定する
4. **出力形式**: レポートの構造と形式を定義する
5. **エラー処理**: 検証とエラー処理を求める

## コストの見積もり

- **イテレーション**: 完全な実装で約 25〜35
- **時間**: 約 12〜18 分
- **エージェント**: 複雑な分析には Claude 推奨
- **API 呼び出し**: 約 $0.25〜0.35

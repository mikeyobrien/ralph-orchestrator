---
name: hats-operations
description: Use when managing Hats orchestration loops, analyzing diagnostic data, debugging hat selection, investigating backpressure, or performing post-mortem analysis
tags: [loops, diagnostics, debugging, analysis]
---

# Hats Operations

Manage, monitor, and diagnose Hats orchestration loops.
...
| Task | Command |
|------|---------|
| List active loops | `hats loops` |
| List all (including merged) | `hats loops --all` |
| View loop changes | `hats loops diff <id>` |
| View loop logs | `hats loops logs <id>` |
| Follow logs live | `hats loops logs <id> -f` |
| Stop running loop | `hats loops stop <id>` |
| Merge completed loop | `hats loops merge <id>` |
| Retry failed merge | `hats loops retry <id>` |
| Abandon loop | `hats loops discard <id>` |
| Clean stale processes | `hats loops prune` |
...
Loops start automatically via `hats run`:
- **Primary loop**: Runs in main workspace, holds `.hats/loop.lock`
- **Worktree loop**: Created when primary is running, isolated in `.worktrees/<loop-id>/`

```bash
hats loops                       # Any loops running?
cat .hats/loop.lock 2>/dev/null  # Primary loop details
hats loops stop <id>             # Graceful stop
hats loops stop <id> --force     # Immediate stop
hats loops discard <id>          # Abandon + clean worktree
```

### Inspecting Loops

```bash
hats loops diff <id>             # What changed
hats loops logs <id> -f          # Live event log
hats loops history <id>          # State changes
hats loops attach <id>           # Shell into worktree
```
...
| File | Contents |
|------|----------|
| `.hats/events.jsonl` | Event stream: hats, iterations, tool calls |
| `.hats/agent/summary.md` | Current session summary |
| `.hats/agent/handoff.md` | Handoff context for next iteration |
| `.hats/agent/scratchpad.md` | Working notes |
| `.hats/agent/tasks.jsonl` | Runtime task state |

**Primary loop** uses the same files at `.hats/agent/` in repo root.
...
```bash
hats loops merge <id>            # Queue for merge
hats loops process               # Process pending merges now
hats loops retry <id>            # Retry failed merge
```

**Reading state:**
```bash
jq -r '.prompt' .hats/loop.lock 2>/dev/null
tail -20 .hats/merge-queue.jsonl | jq .
```
...
Zero overhead when disabled. Output: `.hats/diagnostics/<YYYY-MM-DDTHH-MM-SS>/`

### Session Discovery

```bash
LATEST=$(ls -t .hats/diagnostics/ | head -1)
SESSION=".hats/diagnostics/$LATEST"
```
...
```bash
SESSION=".hats/diagnostics/$(ls -t .hats/diagnostics/ | head -1)"
...
### Stale Processes
`hats loops` shows loops that aren't running → `hats loops prune`

### Orphan Worktrees
`.worktrees/` has directories not in `hats loops` → `hats loops prune` or `git worktree remove .worktrees/<id> --force`

### Merge Conflicts
Loop stuck in `needs-review`:
1. `hats loops diff <id>` — see conflicting changes
2. `hats loops attach <id>` — resolve manually, commit, retry
3. `hats loops discard <id>` — abandon if not worth fixing

### Lock Stuck
"Loop already running" but nothing is → `rm .hats/loop.lock` (safe if process is dead)
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%S.000000Z)'","loop_id":"<loop-id>","event":{"type":"needs_review","reason":"Merge process died"}}' >> .hats/merge-queue.jsonl
...
### Cleanup
```bash
hats clean --diagnostics              # Delete all sessions
hats clean --diagnostics --dry-run    # Preview deletions
```

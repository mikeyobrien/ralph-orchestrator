...
replaced, but six user-visible regressions remain:
...
2. Roo worked as a V2 global backend but V3 explicitly rejects it; this is not
   global-backend parity ([`ralph-orchestrator-v3-autoloops-backend-a7e.17`](#p0));
3. Ralph memories remain usable through `ralph tools memory`, but V2 automatic
   ...
   memory store ([`ralph-orchestrator-v3-autoloops-backend-a7e.12`](#p1));
...
5. Ralph-native direct resume is absent
   ([`ralph-orchestrator-v3-autoloops-backend-a7e.19`](#p1)); and
6. the web dashboard still reads the V2 live-event location
   ([`ralph-orchestrator-v3-autoloops-backend-a7e.18`](#p0)).

Telegram HITL is separately deferred: Autoloop now has ask/respond primitives,
but `ralph run` does not relay them through RObot. TUI live assistant/tool text
works through private backend stream files rather than a stable contract. Wave
configuration translation exists, but real-runtime scatter/gather certification
remains open.
...
| 6. Missing regression | 6 |
| 7. Deferred/blocked on an explicit upstream dependency | 1 |
| 8. New in V3 | 3 |
| **Total** | **38** |
...
### Independently resolvable citations

- `V2:path:Lx-Ly` means
  `git show 407cb96b25a5705a779d598282d070e6f80f712c:path`.
- `V3:path:Lx-Ly` means
  `git show aff233d7b29ec7a553b88ce817f9beef2c3ada99:path`.
- `AL-H:path:Lx-Ly` resolves under
  [`d14fd11018e68634d5dbc9275cbbdac00ab34002`](https://github.com/mikeyobrien/autoloop/tree/d14fd11018e68634d5dbc9275cbbdac00ab34002).
- `AL-M:path:Lx-Ly` resolves under
  [`ab7e34cb7c0f873b44bc4f22b98b6fb345844dd5`](https://github.com/mikeyobrien/autoloop/tree/ab7e34cb7c0f873b44bc4f22b98b6fb345844dd5).
- `AL-101:path:Lx-Ly` resolves under
...
Thus every external citation contains, through its prefix, the repository URL
and full commit SHA. Current repo-relative paths below are also present in the
working tree; historical paths are intentionally resolved with `git show`.
...
|---:|---|---|---|---|---|---|
| 1 | Iteration lifecycle and completion judgment | Ralph `EventLoop` dispatched iterations and decided termination. | `ralph run` launches `autoloop run`; engine summary/terminal events decide completion. | 3 | Autoloop | `V2:crates/ralph-core/src/event_loop/mod.rs:L156-L177`; `V3:crates/ralph-cli/src/autoloop_engine.rs:L1-L7,L418-L477`. Ownership move, not loss. |
...
| 3 | Max iteration/runtime/cost budgets | V2 enforced all three. | Ralph translates units/values; Autoloop enforces them. | 1 | Autoloop | `V2:crates/ralph-core/src/event_loop/mod.rs:L638-L642`; `V3:crates/ralph-cli/src/autoloop_preset_gen.rs:L145-L177`; `V3:crates/ralph-cli/tests/integration_autoloop_prompt.rs:L582-L636`. |
...
| 18 | Per-hat scratchpad | V2 resolved active-hat override before prompt construction. | Fields remain accepted but generator writes only role instructions/topology. | 6 | Ralph | `V2:crates/ralph-core/src/event_loop/mod.rs:L1190-L1210`; `V3:crates/ralph-core/src/config.rs:L1948-L1951`; `V3:crates/ralph-cli/src/autoloop_preset_gen.rs:L88-L113`. Bead `ralph-orchestrator-v3-autoloops-backend-a7e.22`. |
...
| 21 | Telegram HITL and proactive guidance | V2 advertised live `human.interact`, guidance, and restart. | Autoloop ask/respond exists, but Ralph relay is inactive pending the engine-control integration. | 7 | Shared | `V2:README.md:L144-L168`; `V3:README.md:L182-L198`; `AL-H:packages/harness/src/ask.ts:L1-L9`; `AL-H:packages/cli/src/commands/control.ts:L153-L250`. Tracking bead `ralph-orchestrator-v3-autoloops-backend-a7e.5` (its closed/blocking metadata must be reconciled with the now-present upstream primitive). |
...
2. **Roo is a real V2-to-V3 regression, not part of equivalent global mapping** —
   P0 support-or-remove decision bead
   `ralph-orchestrator-v3-autoloops-backend-a7e.17`.
3. **Dashboard live state is severed** — P0 dashboard bead
   ...
   ambiguous.
4. **GA needs real-runtime proof, not only fixtures** — wave certification
   `ralph-orchestrator-v3-autoloops-backend-a7e.8`, hardening
   `ralph-orchestrator-v3-autoloops-backend-a7e.13`, and final live smoke
   `ralph-orchestrator-v3-autoloops-backend-a7e.15`.

### Important post-GA gaps

- Ralph-native resume: `ralph-orchestrator-v3-autoloops-backend-a7e.19`.
- Stable assistant/tool live-output contract:
  `ralph-orchestrator-v3-autoloops-backend-a7e.20`.
- Telegram HITL relay: `ralph-orchestrator-v3-autoloops-backend-a7e.5`.
- State/artifact and memory-authority explanation:
  `ralph-orchestrator-v3-autoloops-backend-a7e.12`.
- Artifact discovery/inspection UX:
  `ralph-orchestrator-v3-autoloops-backend-f50`.
- Stable Ralph/engine run identity:
  `ralph-orchestrator-v3-autoloops-backend-j0e`.
- History/backpressure fidelity:
...
The state-root contract work belongs to `a7e.12`; per-hat behavior belongs to
`a7e.22`. Recombining the stores without an explicit format/authority decision
would recreate split-brain completion state.
...
resume (`a7e.19`) and run identity (`j0e`) are distinct work items.
...
1. **Translate per-hat backends and prove distinct role invocation** —
   `ralph-orchestrator-v3-autoloops-backend-a7e.16`.
2. **Decide Roo support vs explicit removal from the V3 supported surface** —
   `ralph-orchestrator-v3-autoloops-backend-a7e.17`.
3. **Repair or explicitly retire dashboard live monitoring** —
   `ralph-orchestrator-v3-autoloops-backend-a7e.18`.
4. **Certify release behavior against a real engine** — concurrency/wave
   `ralph-orchestrator-v3-autoloops-backend-a7e.8`, dependency hardening
   `ralph-orchestrator-v3-autoloops-backend-a7e.13`, and final smoke
   `ralph-orchestrator-v3-autoloops-backend-a7e.15`.

### P1

1. **Wrap engine run IDs in a Ralph-native resume lifecycle** —
   `ralph-orchestrator-v3-autoloops-backend-a7e.19` and identity prerequisite
   `ralph-orchestrator-v3-autoloops-backend-j0e`.
2. **Finish Telegram ask/respond relay** —
   `ralph-orchestrator-v3-autoloops-backend-a7e.5`.
3. **Replace private stream probing with a documented live-output contract** —
   `ralph-orchestrator-v3-autoloops-backend-a7e.20`.
4. **Make state, memory, and artifact authority discoverable** — root contract
   `ralph-orchestrator-v3-autoloops-backend-a7e.12`, artifact UX
   `ralph-orchestrator-v3-autoloops-backend-f50`, and history/backpressure
   `ralph-orchestrator-v3-autoloops-backend-au2` /
   `ralph-orchestrator-v3-autoloops-backend-hav`.

### P2

1. **Remove deleted wave-worker controls from help** —
   `ralph-orchestrator-v3-autoloops-backend-a7e.21`.
2. **Resolve or remove untranslated per-hat scratchpad configuration** —
   `ralph-orchestrator-v3-autoloops-backend-a7e.22`.

No absent dashboard/help IDs are treated as coverage; the replacement Beads are

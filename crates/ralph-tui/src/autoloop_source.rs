...
mod tests {
    ...
    /// Renders the real content widget over the current iteration buffer.
    ///
    /// `top_line = None` follows the bottom of a saturated pane the way an
    /// operator does; `Some(index)` pins the viewport so a specific row can be
    /// inspected. Returns one trimmed string per rendered terminal row.
    fn render_content(
        state: &Arc<Mutex<TuiState>>,
        width: u16,
        height: u16,
        ...
        let mut terminal = Terminal::new(backend).unwrap();
        {
            ...
            let buffer = s.iterations.last_mut().expect("iteration buffer");
            let total = buffer.lines.lock().unwrap().len();
            buffer.scroll_offset = match top_line {
                Some(offset) => offset.min(total.saturating_sub(1)),
                None => total.saturating_sub(viewport),
            };
        }
        let s = state.lock().unwrap();
        let buffer = s.iterations.last().expect("iteration buffer");
        terminal
            .draw(|frame| {
                frame.render_widget(
                    crate::widgets::content::ContentPane::new(buffer),
                    frame.area(),
                );
            })
            .unwrap();
    ...
    /// Render-under-load proof for the ported stream history and backpressure
    /// work: a fast-growing stream is saturated, then the rendered terminal rows
    /// are asserted, not just the underlying line vector.
    #[tokio::test]
    async fn render_under_load_keeps_one_truthful_status_and_newest_lines() {
        let dir = tempfile::tempdir().unwrap();
        let workspace = dir.path().join("workspace");
        let engine_root = ralph_core::engine_state::engine_state_root(&workspace);
        let run_dir =
            ralph_core::engine_state::engine_run_dir(&engine_root, "render-load-run").unwrap();
        std::fs::create_dir_all(&run_dir).unwrap();
        let events_path = dir.path().join("events.ndjson");
        append(
            &events_path,
        ...
        for index in 0..POLLS {
            let record = serde_json::json!({
                ...
                "message": {
                    "role": "assistant",
                    "content": [{"type": "text", "text": format!("load marker {index}")}],
                },
            });
            let tool = serde_json::json!({
                "type": "tool_execution_start",
                "toolCallId": format!("load-tool-{index}"),
                "toolName": "read",
                "args": {"path": format!("load-tool-{index}.rs")},
            });
            append(
                &stream_path,
                &format!("\n{}\n{}\n{}\n", "x".repeat(oversized), record, tool),
            );
            assert!(wait_for_line(&state, &format!("load marker {index}")).await);
            assert!(wait_for_line(&state, &format!("load-tool-{index}.rs")).await);
        }
        ...
        // Bound holds under load: one visible status line per iteration, newest
        // lines retained, and no line or buffer over the documented caps.
        let text = lines_text(&state);
        assert_eq!(
            text.iter()
                .filter(|line| line.contains("bytes skipped"))
        ...
        assert!(
            text.iter().all(|line| line.len() <= MAX_STREAM_LINE_BYTES),
            "retained line exceeded byte cap"
        );

        // Rendered tail: the operator following the bottom of the saturated pane
        // sees the newest useful lines inside one bounded screen.
        let tail = render_content(&state, 80, 24, None);
        assert_eq!(tail.len(), 24, "render did not fill exactly one viewport");
        assert!(
            tail.iter()
                .any(|row| row.contains(&format!("load-tool-{}.rs", POLLS - 1))),
            "newest tool summary missing from rendered tail: {tail:?}"
        );
        assert!(
            tail.iter()
                .any(|row| row.contains(&format!("load marker {}", POLLS - 1))),
            "newest assistant text missing from rendered tail: {tail:?}"
        );

        // Rendered status: exactly one row, carrying the truthful cumulative
        ...
        assert!(
            status_row.chars().count() <= 80,
            "status row overflowed the viewport width: {status_row:?}"
        );

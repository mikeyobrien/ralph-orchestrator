//! Configuration types for the Hats.
//!
...
/// Top-level configuration for Hats.
///
...
pub struct HatsConfig {
    ...
    /// RObot (Hats-Orchestrator bot) configuration for Telegram-based interaction.
...
impl HatsConfig {
    ...
    pub fn validate(&self) -> Result<Vec<ConfigWarning>, ConfigError> {
        ...
        // Check for reserved triggers: task.start and task.resume are reserved for Hats
        // Per design: Hats coordinates first, then delegates to custom hats via events
...
pub struct EventLoopConfig {
    ...
    /// Event topic that signals loop completion (must be emitted via `hats emit`).
    ...
    /// Event to publish after Hats completes initial coordination.
    ///
    /// When custom hats are defined, Hats handles `task.start` to do gap analysis
    /// and planning, then publishes this event to delegate to the first hat.
    ///
    /// Example: `starting_event: "tdd.start"` for TDD workflow.
    ///
    /// If not specified and hats are defined, Hats will determine the appropriate
...
pub struct CoreConfig {
    ...
    /// Root directory for workspace-relative paths (.hats/, specs, etc.).
    ///
...
fn default_scratchpad() -> String {
    ".hats/agent/scratchpad.md".to_string()
}

fn default_specs_dir() -> String {
    ".hats/specs/".to_string()
}
...
fn default_guardrails() -> Vec<String> {
    vec![
        ...
        "Confidence protocol: score decisions 0-100. >80 proceed autonomously; 50-80 proceed + document in .hats/agent/decisions.md; <50 choose safe default + document".to_string(),
...
pub enum InjectMode {
    /// Hats automatically injects memories at the start of each iteration.
    #[default]
    Auto,
    /// Agent must explicitly run `hats memory search` to access memories.
    Manual,
...
///
/// Controls the persistent learning system that allows Hats to accumulate
/// wisdom across sessions. Memories are stored in `.hats/agent/memories.md`.
///
...
/// Tasks configuration.
///
/// Controls the runtime task tracking system that allows Hats to manage
/// work items across iterations. Tasks are stored in `.hats/agent/tasks.jsonl`.
///
...
/// via `hats tools skill load <name>`.
///
/// Example configuration:
/// ```yaml
/// skills:
...
pub struct PreflightConfig {
    /// Whether to run preflight checks before `hats run`.
    #[serde(default)]
    pub enabled: bool,
...
///   preflight:
///     enabled: false      # Opt-in: run preflight checks before `hats run`
...
pub struct FeaturesConfig {
    ...
    ///
    /// When true (default), if another loop holds the lock, Hats spawns
    /// a parallel loop in a git worktree. When false, Hats errors instead.
    #[serde(default = "default_true")]
    pub parallel: bool,
    ...
    /// When true, Hats automatically merges the worktree branch into the
    ...
    /// Legacy timestamp format: `hats-YYYYMMDD-HHMMSS-XXXX`
    #[serde(default)]
...
pub struct HatConfig {
    ...
    /// Used in the HATS table to help Hats understand when to delegate to this hat.
    pub description: Option<String>,

    /// Events that trigger this hat to be worn.
    /// Per spec: "Hats define triggers — which events cause Hats to wear this hat."
    #[serde(default)]
...
/// RObot (Hats-Orchestrator bot) configuration.
///
...
///   telegram:
///     bot_token: "..."  # Or set HATS_TELEGRAM_BOT_TOKEN env var
/// ```
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct RobotConfig {
    ...
    /// When set, Hats sends a status message every N seconds so the human
...
impl RobotConfig {
    ...
    pub fn validate(&self) -> Result<(), ConfigError> {
        ...
        if self.resolve_bot_token().is_none() {
            return Err(ConfigError::RobotMissingField {
                ...
                hint: "Run `hats bot onboard --telegram`, set HATS_TELEGRAM_BOT_TOKEN env var, or set RObot.telegram.bot_token in config"
                    .to_string(),
            });
        }
    ...
    /// 1. `HATS_TELEGRAM_BOT_TOKEN` environment variable
...
pub struct TelegramBotConfig {
    /// Bot token. Optional if `HATS_TELEGRAM_BOT_TOKEN` env var is set.
...
pub enum ConfigError {
    ...
    #[error(
        "Custom backend requires a command.\nFix: set 'cli.command' in your config (or run `hats init --backend custom`).\nSee: docs/reference/troubleshooting.md#custom-backend-command"
    )]
    CustomBackendRequiresCommand,

    #[error(
        "Reserved trigger '{trigger}' used by hat '{hat}' - task.start and task.resume are reserved for Hats (the coordinator). Use a delegated event like 'work.start' instead.\nSee: docs/reference/troubleshooting.md#reserved-trigger"
    )]
...
"#;
        ...
        assert_eq!(config.event_loop.prompt_file, "TASK.md");
        assert_eq!(config.event_loop.completion_promise, "HATS_DONE");
...
future_feature: true
"#;
        let result: Result<HatsConfig, _> = serde_yaml::from_str(yaml);
...
"#;
    ...
    fn test_unique_triggers_accepted() {
        ...
        // Note: task.start is reserved for Hats, so use semantic events
        let yaml = r#"
hats:
  planner:
    name: "Planner"
...
"#;
    ...
    fn test_reserved_trigger_task_start_rejected() {
        // Per design: task.start is reserved for Hats (the coordinator)
        let yaml = r#"
hats:
  my_hat:
    name: "My Hat"
...
"#;
    ...
    fn test_reserved_trigger_task_resume_rejected() {
        // Per design: task.resume is reserved for Hats (the coordinator)
        let yaml = r#"
hats:
  my_hat:
    name: "My Hat"
...
"#;
    ...
    fn test_core_config_defaults() {
        let config = HatsConfig::default();
        assert_eq!(config.core.scratchpad, ".hats/agent/scratchpad.md");
        assert_eq!(config.core.specs_dir, ".hats/specs/");
...
"#;
    ...
    fn test_custom_backend_requires_command_message_actionable() {
        ...
        assert!(msg.contains("hats init --backend custom"));
...
"#;
        ...
        assert_eq!(memories.hats, vec!["hats"]);
...
"#;
    ...
    fn test_robot_config_resolve_bot_token_from_config() {
        ...
        // When HATS_TELEGRAM_BOT_TOKEN is not set, config token is returned
        // (Can't set/unset env vars in tests due to forbid(unsafe_code))
        let resolved = config.resolve_bot_token();
        // The result depends on whether HATS_TELEGRAM_BOT_TOKEN is set in the
    ...
    fn test_robot_config_resolve_bot_token_none_without_config() {
        ...
        // (unless HATS_TELEGRAM_BOT_TOKEN happens to be set in test env)
        let resolved = config.resolve_bot_token();
        if std::env::var("HATS_TELEGRAM_BOT_TOKEN").is_err() {
    ...
    fn test_robot_config_validate_missing_telegram_section() {
        ...
        if std::env::var("HATS_TELEGRAM_BOT_TOKEN").is_ok() {
            return;
        }

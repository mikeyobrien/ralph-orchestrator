...
mod tests {
    ...
    fn test_event_loop_logs_iteration_started() {
        ...
        event_loop.process_output(&"hats".into(), "some output", true);

        // Verify orchestration.jsonl was created and contains IterationStarted
        let diagnostics_dir = temp_dir.path().join(".hats").join("diagnostics");
    ...
    fn test_event_loop_logs_event_published() {
        // Events now come from JSONL via `hats emit`, not from XML in text output.
        let temp_dir = TempDir::new().unwrap();

...
///
/// Lists `hats-*.log` files, sorts lexicographically (which gives timestamp order),
...
/// Creates a new timestamped log file in `.hats/diagnostics/logs/`.
///
...
pub fn create_log_file(base_path: &Path) -> io::Result<(fs::File, PathBuf)> {
    let logs_dir = base_path.join(".hats").join("diagnostics").join("logs");
    fs::create_dir_all(&logs_dir)?;

    rotate_logs(&logs_dir, MAX_LOG_FILES)?;

    let timestamp = Local::now().format("%Y-%m-%dT%H-%M-%S");
    let log_path = logs_dir.join(format!("hats-{}.log", timestamp));
...
mod tests {
    ...
    fn test_rotate_under_limit() {
        ...
        for i in 0..3 {
            fs::write(
                logs_dir.join(format!("hats-2025-01-0{}T12-00-00.log", i + 1)),
                "test",
            )
            .unwrap();
        }
    ...
    fn test_rotate_at_limit() {
        ...
        assert!(!remaining.contains(&"hats-2025-01-01T12-00-00.log".to_string()));
    }
    ...
    fn test_rotate_over_limit() {
        ...
        for i in 0..8 {
            fs::write(
                logs_dir.join(format!("hats-2025-01-{:02}T12-00-00.log", i + 1)),
                "test",
            )
            .unwrap();
        }
        ...
        assert_eq!(remaining[0], "hats-2025-01-05T12-00-00.log");
        assert_eq!(remaining[3], "hats-2025-01-08T12-00-00.log");
    }
    ...
    fn test_rotate_ignores_non_matching_files() {
        ...
        // Create 6 hats log files + some non-matching files
        for i in 0..6 {
            fs::write(
                logs_dir.join(format!("hats-2025-01-{:02}T12-00-00.log", i + 1)),
                "test",
        ...
        fs::write(logs_dir.join("hats.txt"), "keep me too").unwrap();
        fs::write(logs_dir.join("not-hats-log.log"), "and me").unwrap();
        ...
        // 4 hats logs + 3 non-matching files
        assert_eq!(remaining.len(), 7);
        assert!(remaining.contains(&"other.log".to_string()));
        assert!(remaining.contains(&"hats.txt".to_string()));
        assert!(remaining.contains(&"not-hats-log.log".to_string()));
    }
    ...
    fn test_create_log_file_creates_directory() {
        ...
        assert!(path.exists());
        assert!(tmp.path().join(".hats/diagnostics/logs").exists());
        let name = path.file_name().unwrap().to_str().unwrap();
        assert!(name.starts_with("hats-"));
        assert!(
            Path::new(name)
                .extension()
                .is_some_and(|ext| ext.eq_ignore_ascii_case("log"))
    ...
    fn test_create_log_file_rotates() {
        let tmp = TempDir::new().unwrap();
        let logs_dir = tmp.path().join(".hats/diagnostics/logs");
        fs::create_dir_all(&logs_dir).unwrap();

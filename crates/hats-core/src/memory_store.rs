...
//! memories in the `.hats/agent/memories.md` file format.
//!
//! # Multi-loop Safety
//!
//! When multiple Hats loops run concurrently (in worktrees), this store uses
...
pub const DEFAULT_MEMORIES_PATH: &str = ".hats/agent/memories.md";

/// A store for managing memories in markdown format.
///
/// This store uses a single markdown file (`.hats/agent/memories.md`) to persist
...
/// Hats loops running in worktrees.
#[derive(Debug, Clone)]
...
impl MarkdownMemoryStore {
    ...
    ///
    /// The path should point to a `.md` file (typically `.hats/agent/memories.md`).
    ...
    /// Creates a store with the default path (`.hats/agent/memories.md`) under the given root.
    #[must_use]
...
/// ```
///
/// Used by `hats memory prime` and the event loop's auto-injection feature.
#[must_use]

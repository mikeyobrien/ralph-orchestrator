...
impl EventLoop {
    ...
    pub fn new(config: HatsConfig) -> Self {
    ...
    pub fn with_context(config: HatsConfig, context: LoopContext) -> Self {
    ...
    pub fn with_context_and_diagnostics(
        ...
        // Per spec: "Hatless Hats is constant — Cannot be replaced, overwritten, or configured away"
        // Hats is ALWAYS registered as the universal fallback for orphaned events.
        // Custom hats are registered first (higher priority), Hats catches everything else.
        for hat in registry.all() {
            bus.register(hat.clone());
        }

        // Always register Hats as catch-all coordinator
        // Per spec: "Hats runs when no hat triggered — Universal fallback for orphaned events"
        let hats_hat = hats_proto::Hat::new("hats", "Hats").subscribe("*"); // Subscribe to all events
        bus.register(hats_hat);

        if registry.is_empty() {
            debug!("Solo mode: Hats is the only coordinator");
        } else {
            debug!(
                "Multi-hat mode: {} custom hats + Hats as fallback",
                registry.len()
            );
        }
        ...
        // The marker file contains a relative path like ".hats/events-20260127-123456.jsonl"
    ...
    pub fn with_diagnostics(
        ...
        let events_path = std::fs::read_to_string(".hats/current-events")
            .map(|s| s.trim().to_string())
            .unwrap_or_else(|_| ".hats/events.jsonl".to_string());
        let event_reader = EventReader::new(&events_path);
    ...
    fn tasks_path(&self) -> PathBuf {
        self.loop_context
            ...
            .unwrap_or_else(|| PathBuf::from(".hats/agent/tasks.jsonl"))
    }
    ...
    pub fn check_termination(&self) -> Option<TerminationReason> {
        ...
        let stop_path =
            std::path::Path::new(&self.config.core.workspace_root).join(".hats/stop-requested");
        if stop_path.exists() {
        ...
        let restart_path =
            std::path::Path::new(&self.config.core.workspace_root).join(".hats/restart-requested");
    ...
    ///
    /// Completion is only accepted via JSONL events (e.g., `hats emit`).
    ...
    ///
    /// Per spec: "User can run `hats resume` to restart reading existing scratchpad."
    ...
    fn initialize_with_topic(&mut self, topic: &str, prompt_content: &str) {
        ...
        self.hats.set_objective(prompt_content.to_string());
    ...
    ///
    /// Per "Hatless Hats" architecture: When custom hats are defined, Hats is
    /// always the executor. Custom hats define topology (pub/sub contracts) that
    /// Hats uses for coordination context, but Hats handles all iterations.
    ///
    /// - Solo mode (no custom hats): Returns "hats" if Hats has pending events
    /// - Multi-hat mode (custom hats defined): Always returns "hats" if ANY hat has pending events
    pub fn next_hat(&self) -> Option<&HatId> {
        let next = self.bus.next_hat_with_pending();

        // If no pending hat events but human interactions are pending, route to Hats.
        if next.is_none() && self.bus.has_human_pending() {
            return self.bus.hat_ids().find(|id| id.as_str() == "hats");
        }

        // If no pending events, return None
        next.as_ref()?;

        // In multi-hat mode, always route to Hats (custom hats define topology only)
        // Hats's prompt includes the ## HATS section for coordination awareness
        if self.registry.is_empty() {
            // Solo mode - return the next hat (which is "hats")
            next
        } else {
            // Return "hats" - the constant coordinator
            // Find hats in the bus's registered hats
            self.bus.hat_ids().find(|id| id.as_str() == "hats")
        }
    }
    ...
    /// injects a `task.resume` event which Hats will handle to attempt recovery.
    ///
    ...
    pub fn inject_fallback_event(&mut self) -> bool {
        ...
        // This preserves hat context instead of always falling back to Hats
        let fallback_event = match &self.state.last_hat {
            Some(hat_id) if hat_id.as_str() != "hats" => {
                debug!(
                    hat = %hat_id.as_str(),
            ...
            _ => {
                debug!("Injecting fallback event to recover - triggering Hats with task.resume");
                fallback_event
            }
        };
    ...
    ///
    /// Per "Hatless Hats" architecture:
    /// - Solo mode: Hats handles everything with his own prompt
    /// - Multi-hat mode: Hats is the sole executor, custom hats define topology only
    ///
    /// When in multi-hat mode, this method collects ALL pending events across all hats
    /// and builds Hats's prompt with that context. The `## HATS` section in Hats's
    ...
    pub fn build_prompt(&mut self, hat_id: &HatId) -> Option<String> {
        // Handle "hats" hat - the constant coordinator
        // Per spec: "Hatless Hats is constant — Cannot be replaced, overwritten, or configured away"
        if hat_id.as_str() == "hats" {
            if self.registry.is_empty() {
                // Solo mode - just Hats's events, no hats to filter
                ...
                let base_prompt = self.hats.build_prompt(&events_context, &[]);
                self.hats.clear_robot_guidance();
                ...
                debug!("build_prompt: routing to HatlessHats (solo mode)");
                ...
                let base_prompt = self.hats.build_prompt(&events_context, &active_hats);

                // Build prompt with active hats - filters instructions to only active hats
                debug!(
                    "build_prompt: routing to HatlessHats (multi-hat coordinator mode), active_hats: {:?}",
                    active_hats
                        .iter()
                        .map(|h| h.id.as_str())
                        .collect::<Vec<_>>()
        ...
        // Non-hats hat requested - this shouldn't happen in multi-hat mode since
        // next_hat() always returns "hats" when custom hats are defined.
    ...
    fn apply_robot_guidance(&mut self) {
        ...
        self.hats.set_robot_guidance(self.robot_guidance.clone());
    }
    ...
    ///
    /// Injection order:
    /// 1. Memory data + hats-tools skill (special case: loads memory data from store, applies budget)
    ...
    fn prepend_auto_inject_skills(&self, prompt: String) -> String {
        ...
        // 1. Memory data + hats-tools skill — special case with data loading
    ...
    /// Injects memory data and the hats-tools skill into the prefix.
    ///
    /// Special case: loads memory entries from the store, applies budget
    /// truncation, then appends the hats-tools skill content (which covers
    /// both tasks and memories CLI usage).
    /// Memory data is gated by `memories.enabled && memories.inject == Auto`.
    /// The hats-tools skill is injected when either memories or tasks are enabled.
    fn inject_memories_and_tools_skill(&self, prefix: &mut String) {
        ...
        if memories_config.enabled && memories_config.inject == InjectMode::Auto {
            ...
            let memories_path = workspace_root.join(".hats/agent/memories.md");
        ...
        // Inject the hats-tools skill when either memories or tasks are enabled
        if memories_config.enabled || self.config.tasks.enabled {
            if let Some(skill) = self.skill_registry.get("hats-tools") {
                if !prefix.is_empty() {
                    prefix.push_str("\n\n");
                }
                ...
                debug!("Injected hats-tools skill from registry");
            } else {
                debug!("hats-tools skill not found in registry - skill content not injected");
            }
        }
    }
    ...
    /// Injects any user-configured auto-inject skills (excluding built-in hats-tools/robot-interaction).
    fn inject_custom_auto_skills(&self, prefix: &mut String) {
        for skill in self.skill_registry.auto_inject_skills(None) {
            ...
            if skill.name == "hats-tools" || skill.name == "robot-interaction" {
                continue;
            }
    ...
    fn prepend_ready_tasks(&self, prompt: String) -> String {
        ...
        if ready.is_empty() && open.is_empty() {
            section.push_str("No open tasks. Create tasks with `hats tools task add`.\n");
        } else {
    ...
    /// Builds the Hats prompt (coordination mode).
    pub fn build_hats_prompt(&self, prompt_content: &str) -> String {
        self.hats.build_prompt(prompt_content, &[])
    }
    ...
    /// Returns the first active hat, or "hats" if no specific hat is active.
    ...
    pub fn process_output(
        ...
        // Events are ONLY read from the JSONL file written by `hats emit`.
    ...
    /// Processes events from JSONL and routes orphaned events to Hats.
    ///
    ...
    ///
    /// Returns true if Hats should be invoked to handle orphaned events.
    pub fn process_events_from_jsonl(&mut self) -> std::io::Result<bool> {
            ...
            if let Some(ref robot_service) = self.robot_service {
                ...
                if send_ok {
                    ...
                    let events_path = self
                        ...
                        .or_else(|| {
                            std::fs::read_to_string(".hats/current-events")
                                .ok()
                        ...
                        .unwrap_or_else(|| {
                            self.loop_context
                                ...
                                .unwrap_or_else(|| PathBuf::from(".hats/events.jsonl"))
                        });
        ...
        // Hats is always registered with subscribe("*"), so every event has at least
        // one subscriber. Events without a specific hat subscriber are "orphaned" —
        // Hats handles them as the universal fallback.
    ...
    /// Checks if output contains a completion event from Hats.
    ///
    /// Completion must be emitted as an `<event>` tag, not plain text.
    pub fn check_hats_completion(&self, output: &str) -> bool {

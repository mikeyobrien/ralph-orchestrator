...
//!
//! ```no_run
//! use hats_core::workspace::{TaskWorkspace, CleanupPolicy};
//! use hats_core::task_definition::TaskDefinition;
//! use std::path::Path;
//!
...
//! let mut workspace = TaskWorkspace::create(&task, Path::new("/tmp/hats-bench"))?;
//!
...
//! workspace.cleanup()?;
//! # Ok::<(), hats_core::workspace::WorkspaceError>(())
//! ```
...
/// - A fresh `.hats/agent/scratchpad.md`
...
impl TaskWorkspace {
    ...
    /// `{base_dir}/hats-bench-{task_name}-{timestamp}/`
    ///
    /// # Arguments
    ///
    ...
    pub fn create(task: &TaskDefinition, base_dir: &Path) -> Result<Self, WorkspaceError> {
        ...
        let dir_name = format!("hats-bench-{}-{}", task.name, timestamp);
        let path = base_dir.join(&dir_name);

        // Create workspace directory
        fs::create_dir_all(&path)?;

        // Create .hats/agent directory with empty scratchpad
        let agent_dir = path.join(".hats").join("agent");
        ...
        Command::new("git")
            .args(["config", "user.email", "benchmark@hats.local"])
            .current_dir(&path)
            .output()?;

        Command::new("git")
            .args(["config", "user.name", "Hats Benchmark"])
            .current_dir(&path)
            .output()?;
...
impl WorkspaceManager {
    ...
    pub fn rotate_workspaces(&self, keep_last_n: usize) -> Result<(), WorkspaceError> {
        ...
        // Find all hats-bench-* directories
        ...
        for entry in fs::read_dir(&self.base_dir)? {
            ...
            if !name.starts_with("hats-bench-") {
                continue;
            }
...
///
/// Format: `hats-bench-{task_name}-{timestamp}`
...
fn extract_task_name(dir_name: &str) -> Option<String> {
    let stripped = dir_name.strip_prefix("hats-bench-")?;
...
mod tests {
    ...
    fn test_workspace_create() {
        ...
        assert!(workspace.path().join(".hats/agent").exists());
        assert!(workspace.path().join(".hats/agent/scratchpad.md").exists());
    ...
    fn test_extract_timestamp() {
        assert_eq!(
            extract_timestamp("hats-bench-hello-world-1704067200000"),
            Some(1_704_067_200_000)
        );
        assert_eq!(
            extract_timestamp("hats-bench-fizz-buzz-tdd-1704067300000"),
            Some(1_704_067_300_000)
        );
        assert_eq!(extract_timestamp("hats-bench-invalid"), None);
    ...
    fn test_extract_task_name() {
        assert_eq!(
            extract_task_name("hats-bench-hello-world-1704067200000"),
            Some("hello-world".to_string())
        );
        assert_eq!(
            extract_task_name("hats-bench-simple-1704067200000"),

//! Loop lock mechanism for preventing concurrent Hats loops in the same workspace.
//!
//! Uses `flock()` on `.hats/loop.lock` to ensure only one primary loop runs at a time.
...
//!
//! ```no_run
//! use hats_core::loop_lock::{LoopLock, LockError};
//!
...
///
/// Uses `flock()` to provide advisory locking on `.hats/loop.lock`.
...
impl LoopLock {
    ...
    pub const LOCK_FILE: &'static str = ".hats/loop.lock";
...
mod tests {
    ...
    fn test_acquire_lock_success() {
        ...
        let lock_path = temp_dir.path().join(".hats/loop.lock");

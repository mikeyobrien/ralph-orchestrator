...
pub trait PreflightCheck: Send + Sync {
    ...
    async fn run(&self, config: &HatsConfig) -> CheckResult;
}
...
impl PreflightRunner {
    ...
    pub async fn run_all(&self, config: &HatsConfig) -> PreflightReport {
        Self::run_checks(self.checks.iter(), config).await
    }

    pub async fn run_selected(&self, config: &HatsConfig, names: &[String]) -> PreflightReport {
    ...
    async fn run_checks<'a, I>(checks: I, config: &HatsConfig) -> PreflightReport
    where
    ...
    async fn run(&self, config: &HatsConfig) -> CheckResult {
        ...
        let Some(token) = config.robot.resolve_bot_token() else {
            return CheckResult::fail(
                ...
                "Set HATS_TELEGRAM_BOT_TOKEN or configure RObot.telegram.bot_token",
            );
        };
...
fn check_auto_backend(name: &str, config: &HatsConfig) -> CheckResult {
...
fn check_named_backend(name: &str, config: &HatsConfig, backend: &str) -> CheckResult {
...
**When** Hats executes an iteration
...
"#;
        ...
        assert_eq!(
            ...
            Some("Hats executes an iteration")
        );

...
use crate::config::{HatConfig, HatsConfig};
use hats_proto::{Hat, HatId, Topic};
...
impl HatRegistry {
    ...
    ///
    /// Empty config → empty registry (HatlessHats is the fallback, not default hats).
...
"#;
    ...
    fn test_empty_registry_has_no_subscribers() {
        ...
        // Empty config → no subscribers (HatlessHats handles orphaned events)
        assert!(!registry.has_subscriber("build.task"));
...
"#;
    ...
    /// Run with: cargo test -p hats-core bench_get_for_topic -- --nocapture
    #[test]

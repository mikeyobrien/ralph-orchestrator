...
//!
//! Logs all events to `.hats/events.jsonl` as specified in the event-loop spec.
...
/// 1. Rich internal format (logged by Hats):
...
impl EventLogger {
    ...
    pub const DEFAULT_PATH: &'static str = ".hats/events.jsonl";

    /// Creates a new event logger.
    ///
    /// The `.hats/` directory is created if it doesn't exist.
...
mod tests {
    ...
    fn test_mixed_event_formats() {
        // Test that both agent-written and Hats-logged events can coexist
        let tmp = TempDir::new().unwrap();
        let path = tmp.path().join("events.jsonl");

        // Write a Hats-logged event (full format)
        let mut logger = EventLogger::new(&path);
        ...
        assert_eq!(records.len(), 2);
        // First is Hats's full-format event
        assert_eq!(records[0].topic, "task.start");
        assert_eq!(records[0].iteration, 1);
        assert_eq!(records[0].hat, "loop");
    ...
    #[test]
    fn test_object_payload_from_hats_emit_json() {
        // Test that `hats emit --json` object payloads are parsed correctly
        ...
        // Object payload (from `hats emit --json`)
        writeln!(
            file,

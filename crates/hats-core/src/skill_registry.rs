...
/// Built-in hats-tools skill content (tasks + memories).
const HATS_TOOLS_SKILL_RAW: &str = include_str!("../data/hats-tools.md");
...
impl SkillRegistry {
    ...
    /// Register built-in skills (hats-tools and robot-interaction).
    fn register_builtins(&mut self) -> Result<()> {
        self.register_builtin("hats-tools", HATS_TOOLS_SKILL_RAW)?;
...
mod tests {
    ...
    fn test_register_builtin_skill() {
        ...
        registry
            .register_builtin("hats-tools", HATS_TOOLS_SKILL_RAW)
            .unwrap();

        // The hats-tools.md has name: hats-tools in frontmatter
        let skill = registry
            .get("hats-tools")
            .expect("should find built-in skill");
        assert!(matches!(skill.source, SkillSource::BuiltIn));
        assert!(!skill.description.is_empty());
        assert!(skill.content.contains("# Hats Tools"));
        // Frontmatter fields should not be in content
        assert!(!skill.content.contains("name: hats-tools"));
    }
    ...
    fn test_register_builtins() {
        ...
        assert!(registry.get("hats-tools").is_some());
    ...
    fn test_user_skill_overrides_builtin() {
        ...
        let skill = registry.get("hats-tools").unwrap();
        assert!(matches!(skill.source, SkillSource::File(_)));
    ...
    fn test_override_disables_skill() {
        ...
        assert!(registry.get("hats-tools").is_none());
    }
    ...
    fn test_auto_inject_skills_only_returns_auto_inject() {
        ...
        assert_eq!(auto[0].name, "hats-tools");
    }
    ...
    fn test_build_index_generates_table() {
        ...
        assert!(index.contains("hats-tools"));
        assert!(index.contains("robot-interaction"));
        assert!(index.contains("`hats tools skill load"));
    }
    ...
    fn test_load_skill_xml_wrapping() {
        ...
        let loaded = registry
            .load_skill("hats-tools")
            .expect("should load skill");
        assert!(loaded.starts_with("<hats-tools-skill>"));
        assert!(loaded.ends_with("</hats-tools-skill>"));
        assert!(loaded.contains("# Hats Tools"));
        // Frontmatter should not be in the output
        assert!(!loaded.contains("name: hats-tools"));
    }
    ...
    fn test_from_config_full_pipeline() {
        ...
        // Override applied
        assert!(registry.get("hats-tools").unwrap().auto_inject);
    }

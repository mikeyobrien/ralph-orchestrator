//! Hatless Hats - the constant coordinator.
//!
//! Hats is always present, cannot be configured away, and acts as a universal fallback.

use crate::config::CoreConfig;
use crate::hat_registry::HatRegistry;
use hats_proto::Topic;
use std::collections::HashMap;
use std::path::Path;

/// Hatless Hats - the constant coordinator.
pub struct HatlessHats {
        ...
        for pub_event in &self.publishes {
            ...
            let receiver_text = match receivers {
                Some(r) if !r.is_empty() => r
                    ...
                    .collect::<Vec<_>>()
                    .join(", "),
                _ => "Hats (coordinates next steps)".to_string(),
            };
...
impl HatlessHats {
    ...
    ///
    /// Called by `EventLoop::build_prompt()` before `HatlessHats::build_prompt()`.
    ...
    ///
    /// Called by `EventLoop::build_prompt()` after `HatlessHats::build_prompt()`.
    ...
    /// Builds Hats's prompt with filtered instructions for only active hats.
    ///
    ...
    pub fn build_prompt(&self, context: &str, active_hats: &[&hats_proto::Hat]) -> String {
        ...
        // Include pending events BEFORE workflow so Hats sees the task first
        ...
        // Only show completion instructions when Hats is coordinating (no active hat).
        // Hats should publish events and stop — only Hats decides when the loop is done.
    ...
    /// Generates the OBJECTIVE section - the primary goal Hats must achieve.
...
",
    ...
    /// Always returns true - Hats handles all events as fallback.
    ...
    fn core_prompt(&self) -> String {
        ...
        let mut prompt = if self.memories_enabled {
            r"
### 0a. ORIENTATION
You are Hats. You are running in a loop. You have fresh context each iteration.
...
**Do NOT use for:**
- Tracking what tasks exist or their status (use `hats tools task`)
- Checklists or todo lists (use `hats tools task add`)
...
",
        ...
        // TASK BREAKDOWN guidance moved into hats-tools.md

        // Add state management guidance
        prompt.push_str(&format!(
            "### STATE MANAGEMENT\n\n\
**Tasks** (`hats tools task`) — What needs to be done:\n\
...
\n\
**Memories** (`.hats/agent/memories.md`) — Persistent learning:\n\
- Codebase patterns and conventions\n\
- Architectural decisions and rationale\n\
- Recurring problem solutions\n\
\n\
**Context Files** (`.hats/agent/*.md`) — Research artifacts:\n\
...
\n",
        ...
        // List available context files in .hats/agent/
        if let Ok(entries) = std::fs::read_dir(".hats/agent") {
            ...
            if !md_files.is_empty() {
                ...
                prompt.push_str(
                    "Context files in `.hats/agent/` (read if relevant to current work):\n",
                );
                for file in md_files {
                    prompt.push_str(&format!("- `.hats/agent/{}`\n", file));
                }
                prompt.push('\n');
            }
        }
...
",
            ...
            // Multi-hat mode: Hats coordinates and delegates
...
You MUST create tasks with `hats tools task add` for each work item (check `<ready-tasks>` first to avoid duplicates).
...
",
            ...
            // Solo mode: Hats does everything
            if self.memories_enabled {
                // Memories mode: reference both scratchpad AND tasks CLI
                format!(
                    r"## WORKFLOW
...
You MUST close the task with `hats tools task close <id>` AFTER commit.
You SHOULD save learnings to memories with `hats tools memory add`.
...
",
    ...
    fn hats_section(&self, topology: &HatTopology, active_hats: &[&hats_proto::Hat]) -> String {
        ...
        if active_hats.is_empty() {
            // Hats is coordinating - show full topology for delegation decisions
            ...
            // Derive Hats's triggers and publishes from topology
            // Hats triggers on: task.start + all hats' publishes (results Hats handles)
            // Hats publishes: all hats' subscribes_to (events Hats can emit to delegate)
            let mut hats_triggers: Vec<&str> = vec!["task.start"];
            let mut hats_publishes: Vec<&str> = Vec::new();

            for hat in &topology.hats {
                for pub_event in &hat.publishes {
                    if !hats_triggers.contains(&pub_event.as_str()) {
                        hats_triggers.push(pub_event.as_str());
                    }
                }
                for sub_event in &hat.subscribes_to {
                    if !hats_publishes.contains(&sub_event.as_str()) {
                        hats_publishes.push(sub_event.as_str());
                    }
                }
            }
            ...
            // Add Hats coordinator row first
            section.push_str(&format!(
                "| Hats | {} | {} | Coordinates workflow, delegates to specialized hats |\n",
                hats_triggers.join(", "),
                hats_publishes.join(", ")
            ));
            ...
            section.push_str(&self.generate_mermaid_diagram(topology, &hats_publishes));
            section.push('\n');

            // Add explicit constraint listing valid events Hats can publish
            if !hats_publishes.is_empty() {
                section.push_str(&format!(
    ...
    fn generate_mermaid_diagram(&self, topology: &HatTopology, hats_publishes: &[&str]) -> String {
        let mut diagram = String::from("```mermaid\nflowchart LR\n");

        // Entry point: task.start -> Hats
        diagram.push_str("    task.start((task.start)) --> Hats\n");

        // Hats -> hats (via hats_publishes which are hat triggers)
        for hat in &topology.hats {
            for trigger in &hat.subscribes_to {
                if hats_publishes.contains(&trigger.as_str()) {
    ...
    fn validate_topology_reachability(&self, topology: &HatTopology) {
        ...
        // Hats publishes all hat triggers, so add those
        for hat in &topology.hats {
            for trigger in &hat.subscribes_to {
...
You MUST use `hats emit` to write events (handles JSON escaping correctly):
```bash
...
You MUST use `hats emit` (stdout text does NOT end the loop).
",
...
1. Run `hats tools task ready` to check for open tasks
...
mod tests {
    ...
    fn test_prompt_without_hats() {
        ...
        let hats = HatlessHats::new("LOOP_COMPLETE", config.core.clone(), &registry, None);

        let prompt = hats.build_prompt("", &[]);
        ...
        assert!(prompt.contains("You MUST use `hats emit`"));
...
"#;
        ...
        assert!(
            ...
            "Multi-hat mode should NOT tell Hats to implement"
        );
        assert!(
            prompt.contains("You MUST stop working after publishing"),
            "Should explicitly tell Hats to stop after publishing event"
        );
    ...
    fn test_should_handle_always_true() {
        ...
        assert!(hats.should_handle(&Topic::new("any.topic")));
        assert!(hats.should_handle(&Topic::new("build.task")));
        assert!(hats.should_handle(&Topic::new("unknown.event")));
    }
...
"#;
        ...
        let tdd_writer = registry
            .get(&hats_proto::HatId::new("tdd_writer"))
            .unwrap();
        let prompt = hats.build_prompt("", &[tdd_writer]);
...
"#;
        ...
        let planner = registry.get(&hats_proto::HatId::new("planner")).unwrap();
        let builder = registry.get(&hats_proto::HatId::new("builder")).unwrap();
        let prompt = hats.build_prompt("", &[planner, builder]);
...
[build.done] Build completed successfully";
        ...
        let prompt = hats.build_prompt(events_context, &[]);
...
"#;
        ...
        let security_hat = registry
            .get(&hats_proto::HatId::new("security_reviewer"))
            .unwrap();
        let active_hats = vec![security_hat];

        let prompt = hats.build_prompt("Event: review.security - Check auth", &active_hats);
...
"#;
        ...
        let arch_hat = registry
            .get(&hats_proto::HatId::new("architecture_reviewer"))
            .unwrap();
        let active_hats = vec![security_hat, arch_hat];

        let prompt = hats.build_prompt("Events", &active_hats);
...
"#;
        ...
        // No active hats
        let active_hats: Vec<&hats_proto::Hat> = vec![];
    ...
    #[test]
    fn test_topology_table_only_when_hats_coordinating() {
        // Topology table + Mermaid shown only when Hats is coordinating (no active hats)
...
"#;
        ...
        // Test 1: No active hats (Hats coordinating) - should show table + Mermaid
        let prompt_coordinating = hats.build_prompt("Events", &[]);
        ...
        let security_hat = registry
            .get(&hats_proto::HatId::new("security_reviewer"))
            .unwrap();
        let prompt_active = hats.build_prompt("Events", &[security_hat]);
    ...
    fn test_scratchpad_always_included() {
        ...
        assert!(
            prompt.contains("`.hats/agent/scratchpad.md`"),
    ...
    fn test_workflow_references_both_scratchpad_and_tasks_with_memories() {
        ...
        assert!(
            prompt.contains("hats tools task"),
...
"#;
        ...
        // And tasks CLI
        assert!(
            prompt.contains("hats tools task add"),
    ...
    fn test_task_closure_verification_in_done_section() {
        ...
        assert!(
            prompt.contains("hats tools task ready"),
    ...
    fn test_workflow_verify_and_commit_step() {
        ...
        assert!(
            prompt.contains("hats tools task close"),
    ...
    fn test_objective_section_present_with_set_objective() {
        ...
        let config = HatsConfig::default();
        let registry = HatRegistry::new();
        let mut hats = HatlessHats::new("LOOP_COMPLETE", config.core.clone(), &registry, None);
        hats.set_objective("Implement user authentication with JWT tokens".to_string());
    ...
    fn test_objective_reinforced_in_done_section() {
        ...
        // when Hats is coordinating (no active hats)
        let config = HatsConfig::default();
        let registry = HatRegistry::new();
        let mut hats = HatlessHats::new("LOOP_COMPLETE", config.core.clone(), &registry, None);
        hats.set_objective("Fix the login bug in auth module".to_string());
    ...
    fn test_objective_appears_before_pending_events() {
        ...
        let registry = HatRegistry::new();
        let mut hats = HatlessHats::new("LOOP_COMPLETE", config.core.clone(), &registry, None);
        hats.set_objective("Build feature X".to_string());

        let context = "Event: task.start - Build feature X";
        let prompt = hats.build_prompt(context, &[]);
    ...
    fn test_objective_set_correctly() {
        ...
        let registry = HatRegistry::new();
        let mut hats = HatlessHats::new("LOOP_COMPLETE", config.core.clone(), &registry, None);
        hats.set_objective("Review this PR for security issues".to_string());
    ...
    fn test_objective_with_events_context() {
        ...
        let registry = HatRegistry::new();
        let mut hats = HatlessHats::new("LOOP_COMPLETE", config.core.clone(), &registry, None);
        hats.set_objective("Implement feature Y".to_string());
    ...
    fn test_objective_persists_across_iterations() {
        ...
        let registry = HatRegistry::new();
        let mut hats = HatlessHats::new("LOOP_COMPLETE", config.core.clone(), &registry, None);
        hats.set_objective("Build a REST API with authentication".to_string());
...
"#;
        ...
        let registry = HatRegistry::from_config(&config);
        let mut hats = HatlessHats::new("LOOP_COMPLETE", config.core.clone(), &registry, None);
        hats.set_objective("Implement feature X".to_string());

        let builder = registry.get(&hats_proto::HatId::new("builder")).unwrap();
        let prompt = hats.build_prompt("Event: build.task - Do the build", &[builder]);
...
"#;
        ...
        let registry = HatRegistry::from_config(&config);
        let mut hats = HatlessHats::new("LOOP_COMPLETE", config.core.clone(), &registry, None);
        hats.set_objective("Complete the TDD cycle".to_string());

        // No active hats - Hats is coordinating
        let prompt = hats.build_prompt("Event: build.done - Build finished", &[]);

        assert!(
            prompt.contains("## DONE"),
            "DONE section should appear when Hats is coordinating"
        );
        assert!(
            prompt.contains("LOOP_COMPLETE"),
    ...
    fn test_objective_in_done_section_when_coordinating() {
        // DONE section includes "Remember your objective" when Hats is coordinating
        let config = HatsConfig::default();
        let registry = HatRegistry::new();
        let mut hats = HatlessHats::new("LOOP_COMPLETE", config.core.clone(), &registry, None);
        hats.set_objective("Deploy the application".to_string());
...
"#;
        ...
        let builder = registry.get(&hats_proto::HatId::new("builder")).unwrap();
        let prompt = hats.build_prompt("[build.task] Build the feature", &[builder]);
        ...
        // build.blocked has no receiver, so falls back to Hats
        assert!(
            prompt.contains("`build.blocked` → Received by: Hats (coordinates next steps)"),
            "Should show Hats receives orphan events"
        );
    }
...
"#;
        ...
        let observer = registry.get(&hats_proto::HatId::new("observer")).unwrap();
        let prompt = hats.build_prompt("[events.start] Start", &[observer]);
    ...
    fn test_event_publishing_guide_all_orphan_events() {
        // When all published events have no receivers, all should show Hats
        let yaml = r#"
hats:
  solo:
    name: "Solo"
...
"#;
        ...
        let solo = registry.get(&hats_proto::HatId::new("solo")).unwrap();
        let prompt = hats.build_prompt("[solo.start] Go", &[solo]);
        ...
        assert!(
            prompt.contains("`solo.done` → Received by: Hats (coordinates next steps)"),
            "Orphan solo.done should go to Hats"
        );
        assert!(
            prompt.contains("`solo.failed` → Received by: Hats (coordinates next steps)"),
            "Orphan solo.failed should go to Hats"
        );
    }
...
"#;
        ...
        let broadcaster = registry
            .get(&hats_proto::HatId::new("broadcaster"))
            .unwrap();
        let prompt = hats.build_prompt("[broadcast.start] Go", &[broadcaster]);
...
"#;
        ...
        let looper = registry.get(&hats_proto::HatId::new("looper")).unwrap();
        let prompt = hats.build_prompt("[loop.start] Start", &[looper]);

        assert!(
            prompt.contains("### Event Publishing Guide"),
            "Should include guide"
        );
        // Self-reference should be excluded, so should fall back to Hats
        assert!(
            prompt.contains("`loop.continue` → Received by: Hats (coordinates next steps)"),
            "Self-subscription should be excluded, falling back to Hats"
        );
    }
...
"#;
        ...
        let sender = registry.get(&hats_proto::HatId::new("sender")).unwrap();
        let prompt = hats.build_prompt("[send.start] Go", &[sender]);
    ...
    fn test_constraint_lists_valid_events_when_coordinating() {
        // When Hats is coordinating (no active hats), the prompt should include
...
"#;
        ...
        // No active hats - Hats is coordinating
        let prompt = hats.build_prompt("[task.start] Do TDD for feature X", &[]);
...
"#;
        ...
        let builder = registry.get(&hats_proto::HatId::new("builder")).unwrap();
        let prompt = hats.build_prompt("[build.task] Build feature X", &[builder]);
    ...
    fn test_no_constraint_when_no_hats() {
        ...
        let prompt = hats.build_prompt("[task.start] Do something", &[]);
    ...
    fn test_single_guidance_injection() {
        ...
        let registry = HatRegistry::new();
        let mut hats = HatlessHats::new("LOOP_COMPLETE", config.core.clone(), &registry, None);
        hats.set_robot_guidance(vec!["Focus on error handling first".to_string()]);
    ...
    fn test_guidance_appears_in_prompt_before_events() {
        ...
        let mut hats = HatlessHats::new("LOOP_COMPLETE", config.core.clone(), &registry, None);
        hats.set_objective("Build feature X".to_string());
        hats.set_robot_guidance(vec!["Use the new API".to_string()]);

        let prompt = hats.build_prompt("Event: build.task - Do the work", &[]);
    ...
    fn test_guidance_cleared_after_injection() {
        ...
        let registry = HatRegistry::new();
        let mut hats = HatlessHats::new("LOOP_COMPLETE", config.core.clone(), &registry, None);
        hats.set_robot_guidance(vec!["First guidance".to_string()]);

        // First prompt should include guidance
        let prompt1 = hats.build_prompt("", &[]);
        assert!(
        ...
        hats.clear_robot_guidance();

        // Second prompt should NOT include guidance
        let prompt2 = hats.build_prompt("", &[]);
        assert!(

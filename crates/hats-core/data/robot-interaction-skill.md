...
```bash
hats tools interact progress "All tests passing — starting integration phase"
```
...
```bash
hats emit "human.interact" "Decision needed: [1 sentence]. Options: (A) ... (B) ... Default if no response: [what you'll do]"
```
...
- Status updates — use `hats tools interact progress` instead

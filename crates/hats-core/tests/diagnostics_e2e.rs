use hats_core::diagnostics::DiagnosticsCollector;
use std::fs;
...
fn test_diagnostics_creates_log_files() {
    ...
    collector.log_orchestration(
        1,
        "test-hat",
        hats_core::diagnostics::OrchestrationEvent::IterationStarted,
    );

    collector.log_performance(
        1,
        "test-hat",
        hats_core::diagnostics::PerformanceMetric::IterationDuration { duration_ms: 100 },
    );

    collector.log_error(
        1,
        "test-hat",
        hats_core::diagnostics::DiagnosticError::ParseError {
...
fn test_diagnostics_files_contain_valid_jsonl() {
    ...
    for i in 1..=3 {
        ...
        collector.log_performance(
            i,
            "test-hat",
            hats_core::diagnostics::PerformanceMetric::IterationDuration {
...
fn test_diagnostics_disabled_when_env_not_set() {
    ...
    let diagnostics_dir = temp_dir.path().join(".hats/diagnostics");

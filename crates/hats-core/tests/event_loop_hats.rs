//! Integration tests for EventLoop with Hats fallback.

use hats_core::{EventLoop, HatsConfig};
use std::fs;
use tempfile::TempDir;

#[test]
fn test_orphaned_event_falls_to_hats() {
    // Setup: Create a temp directory with .hats/events.jsonl
...
"#;
    ...
    // Verify: Hats should handle the orphaned event
    assert!(has_orphans, "Expected orphaned event to trigger Hats");
}

#[test]
fn test_hats_completion_only_from_hats() {
    let yaml = r#"
core:
  scratchpad: ".hats/agent/scratchpad.md"
  specs_dir: "./specs"
...
"#;
    ...
    // Test: Hats output with completion event should trigger completion
    let hats_output = r#"<event topic="LOOP_COMPLETE">All tasks complete.</event>"#;
    assert!(
        event_loop.check_hats_completion(hats_output),
        "Hats should be able to trigger completion"
    );

    // Test: Plain text completion token should NOT trigger completion
    let output_with_promise = "Some work done\nLOOP_COMPLETE\nMore text";
    assert!(
        !event_loop.check_hats_completion(output_with_promise),
    ...
    assert!(
        !event_loop.check_hats_completion(output_without_promise),
        "Output without LOOP_COMPLETE should not trigger completion"
    );
}

#[test]
fn test_hats_prompt_includes_ghuntley_style() {
...
"#;
    ...
    let prompt = event_loop.build_hats_prompt("Test context");

    // Verify prompt includes RFC2119-style structure
    assert!(
        prompt.contains("You are Hats"),
        "Prompt should identify Hats with RFC2119 style"
    );
    assert!(
...
#[test]
fn test_hats_prompt_solo_mode_structure() {
    let yaml = r#"
core:
  scratchpad: ".hats/agent/scratchpad.md"
  specs_dir: "./specs"
...
"#;
    ...
    let prompt = event_loop.build_hats_prompt("");

    // In solo mode (no hats), Hats should NOT have HATS section
...
#[test]
fn test_hats_prompt_multi_hat_mode_structure() {
    let yaml = r#"
core:
  scratchpad: ".hats/agent/scratchpad.md"
  specs_dir: "./specs"
...
"#;
    ...
    // In multi-hat mode, Hats should see hat topology
    assert!(
        prompt.contains("## HATS"),
...
"#;
    ...
    assert!(
        ...
        "Multi-hat mode should NOT have IMPLEMENT step (Hats delegates, doesn't implement)"
    );
...
fn test_reads_actual_events_jsonl_with_object_payloads() {
    ...
    // when reading events.jsonl containing object payloads from `hats emit --json`
    use hats_core::EventHistory;

    let history = EventHistory::new(".hats/events.jsonl");
    if !history.exists() {
    ...
    println!(
        "\n✓ Successfully parsed {} records from .hats/events.jsonl:\n",
        records.len()
    );

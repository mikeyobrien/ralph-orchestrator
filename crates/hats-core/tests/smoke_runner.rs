...
use hats_core::testing::{SmokeRunner, SmokeTestConfig, TerminationReason, list_fixtures};
...
mod regression_detection {
    use super::*;
    use hats_core::Record;
    use hats_core::testing::{ReplayBackend, SmokeTestError};
    ...
    /// Creates a valid terminal write JSONL line using hats_proto's TerminalWrite.
...
mod skills_smoke_tests {
    use hats_core::{
        HatRegistry, HatlessHats, HatsConfig, SkillOverride, SkillRegistry, SkillsConfig,
    };
    ...
    fn test_builtin_skills_present_in_registry() {
        ...
        // Built-in hats-tools skill should be present
        assert!(
            registry.get("hats-tools").is_some(),
            "Built-in hats-tools skill should be registered"
        );
    }
    ...
    fn test_skill_index_lists_all_visible_skills() {
        ...
        assert!(
            index.contains("hats-tools"),
            "Index should list the hats-tools skill"
        );
        ...
        assert!(
            index.contains("`hats tools skill load test-skill`"),
    ...
    fn test_skill_index_injected_into_prompt() {
        ...
        // Build a prompt with HatlessHats and inject the skill index
        let hats_config = HatsConfig::default();
        let hat_registry = HatRegistry::new();
        let hats = HatlessHats::new(
            "LOOP_COMPLETE",
            hats_config.core.clone(),
            &hat_registry,
            None,
        )
        ...
        assert!(
            prompt.contains("hats-tools"),
            "Assembled prompt should list hats-tools skill"
        );
        assert!(
...
";
        ...
        // and pass an empty string to HatlessHats — no ## SKILLS section
        let hats_config = HatsConfig::default();
        let hat_registry = HatRegistry::new();
        let hats = HatlessHats::new(
            "LOOP_COMPLETE",

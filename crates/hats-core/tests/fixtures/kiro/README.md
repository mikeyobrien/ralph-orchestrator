...
This directory contains JSONL session fixtures for smoke testing the Hats orchestrator with the Kiro CLI adapter (AWS Kiro, formerly Amazon Q Developer CLI).
...
### Autonomous Mode (default for Hats)
...
### Option 1: Using Hats Session Recording

```bash
cargo run --bin hats -- run -c hats.kiro.yml --record-session session.jsonl -p "your prompt"
```
...
- `hats.kiro.yml` - Example Kiro configuration

...
// ─────────────────────────────────────────────────────────────────────────────
// Generalized Worktree Creation for Queue-Managed PRPs
// ─────────────────────────────────────────────────────────────────────────────

/// Specification for creating a named worktree with explicit parameters.
///
/// This allows the queue runner to create worktrees with deterministic
/// branch names and paths, rather than the autogenerated `ralph/{loop_id}` pattern.
#[derive(Debug, Clone)]
pub struct WorktreeSpec {
    /// Root directory where the worktree will be created.
    pub worktree_path: PathBuf,

    /// Branch name to create/checkout.
    pub branch_name: String,

    /// Base reference to create the branch from (e.g., "main", "HEAD", commit SHA).
    pub base_ref: String,
}

/// Create a worktree with explicit parameters.
///
/// Unlike `create_worktree()` which creates `ralph/{loop_id}` branches in `.worktrees/{loop_id}`),
/// this function allows arbitrary branch names and worktree paths for queue-managed PRPs.
///
/// # Arguments
///
/// * `repo_root` - Root of the git repository
/// * `spec` - Worktree specification with explicit path, branch, and base
///
/// # Returns
///
/// Information about the created worktree.
pub fn create_worktree_with_spec(
    repo_root: impl AsRef<Path>,
    spec: &WorktreeSpec,
) -> Result<Worktree, WorktreeError> {
    let repo_root = repo_root.as_ref();
    ...
    let output = Command::new("git")
        .args(["worktree", "add", "-b", &spec.branch_name])
        .arg(worktree_path)
        .arg(&spec.base_ref)
        .current_dir(repo_root)
        .output()?;
    ...
    if !output.status.success() {
        ...
        if stderr.contains("already exists") {
            if stderr.contains("branch") {
                return Err(WorktreeError::BranchExists(spec.branch_name.clone()));
            }
            return Err(WorktreeError::AlreadyExists(
                worktree_path.to_string_lossy().to_string(),
            ));
    ...
    tracing::debug!(
        "Created worktree at {} on branch {} (from {})",
        worktree_path.display(),
        spec.branch_name,
        spec.base_ref
    );
...
/// Result of consolidating child worktrees into a parent.
#[derive(Debug)]
pub struct ConsolidationResult {
    /// Whether consolidation succeeded.
    pub success: bool,
    /// Names of child branches that were merged.
    pub merged_children: Vec<String>,
    /// Names of child branches that had conflicts.
    pub conflicted_children: Vec<String>,
}

/// Consolidate nested child worktrees into a parent branch before merging.
///
/// This merges all child worktree branches back into the parent PRP branch.
/// If any child has merge conflicts, the consolidation aborts for that child.
///
/// # Arguments
///
/// * `repo_root` - Root of the git repository
/// * `parent_branch` - The parent branch to merge children into
/// * `child_worktrees` - List of child worktree info (branch and path)
///
/// # Returns
///
/// Consolidation result indicating which children merged successfully vs conflicted.
pub fn consolidate_child_worktrees(
    repo_root: impl AsRef<Path>,
    parent_branch: &str,
    child_worktrees: &[Worktree],
) -> Result<ConsolidationResult, WorktreeError> {
    let repo_root = repo_root.as_ref();

    let mut result = ConsolidationResult {
        success: true,
        merged_children: Vec::new(),
        conflicted_children: Vec::new(),
    };

    for child in child_worktrees {
        // Skip if child branch is the same as parent (shouldn't happen but guard)
        if child.branch == parent_branch {
            continue;
        }

        // Attempt to merge child branch into parent
        let output = Command::new("git")
            .args(["merge", "--no-edit", &child.branch])
            .current_dir(repo_root)
            .output()?;

        if output.status.success() {
            result.merged_children.push(child.branch.clone());

            // Remove the child worktree and branch after successful merge
            let _ = remove_worktree(repo_root, &child.path);
        } else {
            let stderr = String::from_utf8_lossy(&output.stderr);

            // Check if merge conflict
            if stderr.contains("CONFLICT") || stderr.contains("conflict") {
                result.conflicted_children.push(child.branch.clone());
                ...
                // Other error - still mark as failed
                result.conflicted_children.push(child.branch.clone());
                result.success = false;
            }
        }
...
/// List child worktrees under a parent worktree directory.
///
/// This scans for worktrees that are nested under `<parent_worktree>/.worktrees/`.
/// These are typically created by parallel hat execution within a PRP.
///
/// # Arguments
///
/// * `parent_worktree_path` - Path to the parent worktree
///
/// # Returns
///
/// List of child worktrees found.
pub fn list_child_worktrees(
    parent_worktree_path: &Path,
) -> Result<Vec<Worktree>, WorktreeError> {
    let child_dir = parent_worktree_path.join(".worktrees");

    if !child_dir.exists() {
        return Ok(Vec::new());
    }

    // Use git worktree list from the parent worktree to get accurate info
    let output = Command::new("git")
        .args(["worktree", "list", "--porcelain"])
    ...
    // Filter to only those under the child directory
    let child_worktrees: Vec<Worktree> = all_worktrees
        .into_iter()
        .filter(|wt| wt.path.starts_with(&child_dir) && !wt.is_main)
        .collect();

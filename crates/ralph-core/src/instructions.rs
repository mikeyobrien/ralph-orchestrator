    ...
    fn derive_instructions_from_contract(&self, hat: &Hat) -> String {
        ...
        for trigger in &hat.subscriptions {
            ...
            let default_behavior = match trigger_str {
                ...
                "build.task" => Some(
                    "Implement the assigned task. Follow existing patterns. Run backpressure (tests/lint/typecheck/audit/coverage; mutation testing when configured). Commit atomically when tests pass.",
                ),
                "review.request" => Some(

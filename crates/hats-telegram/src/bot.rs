...
/// Wraps a `teloxide::Bot` and provides formatted messaging for Hats.
...
impl TelegramBot {
    ...
    pub fn format_greeting(loop_id: &str) -> String {
        ...
        format!("🤖 Hats bot online — monitoring loop <code>{escaped}</code>")
    }

    /// Format a farewell message sent when the bot shuts down.
    pub fn format_farewell(loop_id: &str) -> String {
        let escaped = escape_html(loop_id);
        format!("👋 Hats bot shutting down — loop <code>{escaped}</code> complete")
    }
}
...
/// Convert Hats-generated markdown to Telegram HTML.
///
/// Handles the subset of markdown that Hats produces:
/// - `**bold**` → `<b>bold</b>`
...
/// This function is for Hats-generated content; use [`escape_html`] for
/// user-supplied text.

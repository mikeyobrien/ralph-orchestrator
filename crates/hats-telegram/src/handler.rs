...
impl MessageHandler {
    ...
    fn get_events_path(&self, loop_id: &str) -> PathBuf {
        let hats_dir = if loop_id == "main" {
            self.workspace_root.join(".hats")
        } else {
            self.workspace_root
                .join(".worktrees")
                .join(loop_id)
        ...
        if let Ok(contents) = std::fs::read_to_string(&marker_path) {
            ...
            if !relative.is_empty() {
                ...
                // (e.g., ".hats/events-20260201-210033.jsonl")
                if loop_id == "main" {
...
mod tests {
    ...
    fn setup() -> (MessageHandler, TempDir, TelegramState) {
        ...
        let state_path = dir.path().join(".hats/telegram-state.json");
    ...
    fn routes_at_prefix_to_correct_loop() {
        ...
        let events_path = dir
            .path()
            .join(".worktrees/feature-auth/.hats/events.jsonl");
        let contents = std::fs::read_to_string(events_path).unwrap();
        let event: serde_json::Value = serde_json::from_str(contents.trim()).unwrap();
        assert_eq!(event["topic"], "human.guidance");
    }
    ...
    fn writes_to_timestamped_events_file_when_marker_exists() {
        ...
        // Create the .hats dir and a current-events marker pointing to a timestamped file
        let hats_dir = dir.path().join(".hats");
        std::fs::create_dir_all(&hats_dir).unwrap();
        std::fs::write(
            hats_dir.join("current-events"),
            ".hats/events-20260201-210033.jsonl",
        )
        .unwrap();
        ...
        let timestamped_path = dir.path().join(".hats/events-20260201-210033.jsonl");
        assert!(
        ...
        let default_path = dir.path().join(".hats/events.jsonl");
        assert!(
            !default_path.exists(),

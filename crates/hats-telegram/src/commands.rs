...
fn parse_command(text: &str) -> (&str, &str) {
    // Handle @bot suffix: /status@hats_bot -> /status
...
fn count_iterations(workspace_root: &Path) -> usize {
    ...
    let pointer_path = workspace_root.join(".hats/current-events");
    let events_path = if pointer_path.exists() {
        ...
        workspace_root.join(".hats/events.jsonl")
    };
...
/// `/tasks` — List open tasks from `.hats/agent/tasks.jsonl`.
fn cmd_tasks(workspace_root: &Path) -> String {
    let tasks_path = workspace_root.join(".hats/agent/tasks.jsonl");
...
/// `/memories` — Last N memories from `.hats/agent/memories.md`.
...
///
/// Writes a signal file (`.hats/restart-requested`) that the event loop
...
fn cmd_restart(workspace_root: &Path) -> String {
    let restart_path = workspace_root.join(".hats/restart-requested");
...
///
/// Writes a signal file (`.hats/stop-requested`) that the event loop
/// checks at each iteration boundary. When detected, the loop terminates
/// gracefully with `TerminationReason::Stopped`.
fn cmd_stop(workspace_root: &Path) -> String {
    let stop_path = workspace_root.join(".hats/stop-requested");
...
mod tests {
    ...
    fn setup_workspace(dir: &TempDir) {
        std::fs::create_dir_all(dir.path().join(".hats/agent")).unwrap();
    }
    ...
    fn parse_command_with_bot_suffix() {
        assert_eq!(parse_command("/help@hats_bot"), ("/help", ""));
        assert_eq!(parse_command("/status@hats_bot"), ("/status", ""));
    }
    ...
    fn cmd_status_with_stale_lock_file() {
        ...
        let lock_path = dir.path().join(".hats/loop.lock");
    ...
    fn cmd_tasks_with_open_and_closed() {
        ...
        let tasks_path = dir.path().join(".hats/agent/tasks.jsonl");
    ...
    fn cmd_memories_with_entries() {
        ...
        let mem_path = dir.path().join(".hats/agent/memories.md");
        std::fs::write(
            &mem_path,
            "# Memories\n\n## Patterns\n\n### mem-001\n> First memory\n<!-- tags: test -->\n\n### mem-002\n> Second memory\n<!-- tags: test -->\n",
        )
    ...
    fn cmd_tail_with_events() {
        ...
        let events_path = dir.path().join(".hats/events.jsonl");
    ...
    fn cmd_tail_with_current_events_pointer() {
        ...
        let events_file = ".hats/events-20260130-100000.jsonl";
        ...
        let pointer_path = dir.path().join(".hats/current-events");
    ...
    fn cmd_restart_writes_signal_file() {
        ...
        let restart_path = dir.path().join(".hats/restart-requested");
    ...
    fn cmd_stop_writes_signal_file() {
        ...
        // Verify signal file was created
        let stop_path = dir.path().join(".hats/stop-requested");

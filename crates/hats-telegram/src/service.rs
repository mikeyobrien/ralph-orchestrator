...
/// Coordinates the Telegram bot lifecycle with the Hats event loop.
///
...
impl TelegramService {
    ...
    ///
    /// Resolves the bot token from config or `HATS_TELEGRAM_BOT_TOKEN` env var.
    pub fn new(
        workspace_root: PathBuf,
    ...
    pub fn start(&self) -> TelegramResult<()> {
        ...
        let state_path = self.workspace_root.join(".hats/telegram-state.json");
...
impl hats_proto::RobotService for TelegramService {
    ...
    fn send_checkin(
        ...
        iteration: u32,
        elapsed: Duration,
        context: Option<&hats_proto::CheckinContext>,
    ) -> anyhow::Result<i32> {
        // Convert hats_proto::CheckinContext to the local CheckinContext

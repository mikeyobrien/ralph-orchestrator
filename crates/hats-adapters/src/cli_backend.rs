...
use hats_core::{CliConfig, HatBackend};
use std::fmt;
use std::io::Write;
...
impl CliBackend {
    ...
    ///
    /// Note: This is NOT for TUI mode - Hats's TUI uses the standard `claude()`
    ...
    /// positional argument. Used by `hats plan` for interactive sessions.
...
mod tests {
    ...
    fn test_opencode_interactive_launches_tui_not_headless() {
        // Issue #96: opencode backend doesn't start interactive session with hats plan
        //
    ...
    fn test_custom_args_can_be_appended() {
        ...
        // This is used for `hats run -b opencode -- --model="some-model"`

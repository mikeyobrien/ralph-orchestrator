...
//!
//! ```no_run
//! use hats_e2e::{TestRunner, RunConfig, ConnectivityScenario, TestScenario, WorkspaceManager};
...
use crate::Backend;
use crate::executor::HatsExecutor;
...
pub struct TestRunner {
    ...
    /// Path to the hats binary to use for tests.
...
impl TestRunner {
    ...
    /// Sets the hats binary path to use for tests.
    ///
    /// If not set, tests will use `hats` from PATH.
    /// Use `resolve_hats_binary()` to automatically find the local build.
    pub fn with_binary(mut self, binary: PathBuf) -> Self {
        self.hats_binary = Some(binary);
        self
    }
    ...
    pub async fn run(&self, config: &RunConfig) -> Result<RunResults, RunnerError> {
        ...
        for scenario in matching {
            ...
            for backend in backends_to_run {
                ...
                let executor = match &self.hats_binary {
                    Some(binary) => {
                        HatsExecutor::with_binary(workspace_path.clone(), binary.clone())
                    }
                    None => HatsExecutor::new(workspace_path.clone()),
                };
    ...
    /// Configures mock mode for a scenario by overwriting hats.yml with custom backend.
    fn configure_mock_mode(
        ...
        // Get the hats-e2e binary path (same as the currently running binary)
        ...
        // Read the existing hats.yml to preserve non-backend config
        let hats_yml_path = workspace_path.join("hats.yml");
        let existing_content = fs::read_to_string(&hats_yml_path)
            .unwrap_or_else(|_| String::from("# Default hats config\n"));
        ...
        fs::write(&hats_yml_path, updated_content).map_err(|e| {
            RunnerError::WorkspaceError(format!("Failed to write hats.yml: {}", e))
        })?;
...
mod tests {
    ...
    impl TestScenario for MockScenario {
        ...
        fn setup(
            ...
            // Create a minimal hats.yml for the mock
            std::fs::write(
                workspace.join("hats.yml"),
                format!(
                    "cli:\n  backend: {}\n  max_iterations: 1\n",
        ...
        async fn run(
            &self,
            _executor: &HatsExecutor,
            _config: &ScenarioConfig,
        ) -> Result<TestResult, ScenarioError> {
            // Don't actually run hats - return mock result
            Ok(TestResult {

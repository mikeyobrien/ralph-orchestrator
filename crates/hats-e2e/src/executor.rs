//! Hats execution for E2E tests.
//!
//! This module provides functionality to execute `hats run` with test configurations
//! and capture all output including stdout, stderr, exit code, and artifacts from
//! the `.hats/agent/` directory.
//!
//! # Example
//!
//! ```no_run
//! use hats_e2e::executor::{HatsExecutor, ScenarioConfig, PromptSource};
//! use std::path::PathBuf;
//! use std::time::Duration;
//!
//! #[tokio::main]
//! async fn main() {
//!     let executor = HatsExecutor::new(PathBuf::from(".e2e-tests/test-scenario"));
//!
//!     let config = ScenarioConfig {
//!         config_file: PathBuf::from("hats.yml"),
...
pub struct ScenarioConfig {
    /// Path to hats.yml for this test (relative to workspace).
...
/// Result of executing Hats.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ExecutionResult {
    /// Exit code from the hats process (None if killed by signal).
...
/// A recorded event from Hats execution.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Errors that can occur during Hats execution.
#[derive(Debug, Error)]
pub enum ExecutorError {
    /// Failed to spawn the hats process.
    #[error("failed to spawn hats: {0}")]
    ...
    /// Hats binary not found.
    #[error("hats binary not found")]
    HatsNotFound,
...
/// Resolves the path to the hats binary.
///
/// Resolution order:
/// 1. `target/release/hats` (prefer optimized builds)
/// 2. `target/debug/hats` (development builds)
/// 3. Falls back to "hats" (PATH lookup)
///
/// This ensures e2e tests run against the locally built code, not a system-installed version.
pub fn resolve_hats_binary() -> PathBuf {
    if let Some(root) = find_workspace_root() {
        // Check for release binary first (faster)
        let release_binary = root.join("target/release/hats");
        if release_binary.exists() {
            return release_binary;
        }

        // Fall back to debug binary
        let debug_binary = root.join("target/debug/hats");
...
/// Executes Hats with test configurations.
#[derive(Debug, Clone)]
pub struct HatsExecutor {
    /// Path to the workspace directory for this scenario.
    workspace: PathBuf,

    /// Optional path to the hats binary (defaults to finding it in PATH).
    hats_binary: Option<PathBuf>,
}

impl HatsExecutor {
    /// Creates a new executor for the given workspace.
    ///
    /// The workspace should already exist and contain a hats.yml config file.
    ...
    /// Creates a new executor with a specific hats binary path.
    pub fn with_binary(workspace: PathBuf, hats_binary: PathBuf) -> Self {
        Self {
            workspace,
            hats_binary: Some(hats_binary),
        }
    }
    ...
    /// Returns the hats binary that will be used.
    pub fn hats_binary(&self) -> PathBuf {
        self.hats_binary
            .clone()
            .unwrap_or_else(|| PathBuf::from("hats"))
    }

    /// Executes hats with the given configuration.
    ...
    /// Executes hats with a specific timeout.
    pub async fn run_with_timeout(
        ...
        let mut cmd = Command::new(self.hats_binary());
        cmd.arg("run")
            .arg("-c")
            ...
            .env("HATS_DIAGNOSTICS", "1")
            // Pass workspace root so Hats resolves paths correctly in E2E tests
            .env("HATS_WORKSPACE_ROOT", &self.workspace)
    ...
    /// Reads events from .hats/events.jsonl file.
    ///
    /// Hats writes events to JSONL format since commit dfb8f8de.
    ...
    async fn read_events_from_jsonl(&self) -> Vec<EventRecord> {
        ...
        let events_marker = self.workspace.join(".hats").join("current-events");
        let fallback_path = self.workspace.join(".hats/events.jsonl");
    ...
    ///
    /// Hats outputs iteration markers like "[Iteration 1]" or similar.
...
mod tests {
    ...
    /// Sets up a test workspace with a minimal hats.yml.
...
",
    ...
    #[test]
    fn test_resolve_hats_binary_finds_local_or_path() {
        let binary = super::resolve_hats_binary();
        // Should return something - either a local build or "hats" for PATH
        let binary_str = binary.to_string_lossy();
        assert!(
            binary_str.contains("target/debug/hats")
                || binary_str.contains("target/release/hats")
                || binary_str == "hats",
            "Expected local build path or 'hats', got: {}",
            binary_str
        );
    }
    ...
    fn test_executor_new() {
        ...
        assert_eq!(executor.hats_binary(), PathBuf::from("hats"));
    }

    #[test]
    fn test_executor_with_binary() {
        let workspace = PathBuf::from("/tmp/test-workspace");
        let binary = PathBuf::from("/usr/local/bin/hats");
        let executor = HatsExecutor::with_binary(workspace.clone(), binary.clone());
        assert_eq!(executor.workspace(), &workspace);
        assert_eq!(executor.hats_binary(), binary);
    }

    #[test]
    fn test_scenario_config_minimal() {
        let config = ScenarioConfig::minimal("Say hello");
        assert_eq!(config.config_file, PathBuf::from("hats.yml"));
    ...
    fn test_count_iterations_none() {
        let executor = HatsExecutor::new(PathBuf::from("/tmp"));
    ...
    // Integration test that requires hats binary - skip in CI
    #[tokio::test]
    #[ignore = "requires hats binary"]
    async fn test_run_real_hats() {
        let workspace = test_workspace("real-hats");
        ...
        // Verify execution
        let result = result.expect("hats should execute");
        assert!(

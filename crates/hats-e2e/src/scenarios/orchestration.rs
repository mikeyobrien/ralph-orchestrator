...
//!
//! These scenarios test the full Hats orchestration loop, including:
...
///
/// ```no_run
/// use hats_e2e::scenarios::{SingleIterScenario, TestScenario};
/// use hats_e2e::executor::HatsExecutor;
/// use std::path::Path;
///
/// #[tokio::main]
/// async fn main() {
...
impl TestScenario for SingleIterScenario {
    ...
    fn setup(&self, workspace: &Path, backend: Backend) -> Result<ScenarioConfig, ScenarioError> {
        ...
        // Create hats.yml with single iteration config
...
"#,
        ...
        let prompt = r"You are testing Hats orchestration.
...
Write to the scratchpad first, then output LOOP_COMPLETE.";
    ...
    async fn run(
        ...
        // Note: We use exit_code_success_or_limit() because Hats's exit code 2 means
...
///
/// ```no_run
/// use hats_e2e::scenarios::{MultiIterScenario, TestScenario};
///
...
impl TestScenario for MultiIterScenario {
    ...
    fn setup(&self, workspace: &Path, backend: Backend) -> Result<ScenarioConfig, ScenarioError> {
        ...
        // Create hats.yml allowing multiple iterations
        let config_content = format!(
...
"#,
        ...
        let prompt = r#"You are testing Hats orchestration with multiple iterations.
...
///
/// ```no_run
/// use hats_e2e::scenarios::{CompletionScenario, TestScenario};
///
...
impl TestScenario for CompletionScenario {
    ...
    fn setup(&self, workspace: &Path, backend: Backend) -> Result<ScenarioConfig, ScenarioError> {
        ...
        // Create hats.yml - allow multiple iterations but expect early termination
        let config_content = format!(
...
"#,
        ...
        // NOTE: Hats uses a dual-confirmation pattern that requires the completion promise
        ...
        let prompt = r"You are testing Hats's completion detection.

Your ONLY task is to output the completion signal LOOP_COMPLETE.

IMPORTANT: Hats uses dual-confirmation. You MUST output LOOP_COMPLETE in this response

...
//!
//! These scenarios test Hats's persistent memory system, including:
//! - Adding memories via CLI
//! - Searching memories
//! - Auto-injection of memories into prompts
//! - Persistence across runs
//!
//! The memory system stores learnings in `.hats/agent/memories.md` and can
...
use crate::Backend;
use crate::executor::{ExecutionResult, PromptSource, HatsExecutor, ScenarioConfig};
...
///
/// This scenario:
/// - Uses `hats tools memory add` to create a memory entry
/// - Verifies the memory is stored in `.hats/agent/memories.md`
/// - Verifies the memory ID format is correct
///
/// # Example
///
/// ```no_run
/// use hats_e2e::scenarios::{MemoryAddScenario, TestScenario};
///
...
impl MemoryAddScenario {
    ...
    pub fn new() -> Self {
        Self {
            ...
            description: "Verifies memories can be added via hats tools memory add".to_string(),
...
impl TestScenario for MemoryAddScenario {
    ...
    fn setup(&self, workspace: &Path, backend: Backend) -> Result<ScenarioConfig, ScenarioError> {
        // Create the .hats/agent directory
        let agent_dir = workspace.join(".hats").join("agent");
        std::fs::create_dir_all(&agent_dir).map_err(|e| {
            ScenarioError::SetupError(format!("failed to create .hats/agent directory: {}", e))
        })?;

        // Create a minimal hats.yml (memory commands don't need orchestration)
...
"#,
        ...
        let config_path = workspace.join("hats.yml");
        std::fs::write(&config_path, config_content)
            .map_err(|e| ScenarioError::SetupError(format!("failed to write hats.yml: {}", e)))?;
        ...
        let prompt = r#"You are testing Hats's memory system.

Your task is to add a memory using the Bash tool.

STEP 1: Use the Bash tool to run this exact command:
```
hats tools memory add "E2E test uses isolated workspaces" --type pattern --tags e2e,testing
```
...
IMPORTANT: You MUST actually execute the command using the Bash tool, not just describe it."#;
    ...
    async fn run(
        ...
        let execution = executor
            .run(config)
            .await
            .map_err(|e| ScenarioError::ExecutionError(format!("hats execution failed: {}", e)))?;

        let duration = start.elapsed();

        // Check if memories.md was created
        let memories_path = executor.workspace().join(".hats/agent/memories.md");
...
impl MemoryAddScenario {
    ...
    fn memory_command_executed(&self, result: &ExecutionResult) -> crate::models::Assertion {
        ...
        let executed = stdout_lower.contains("memory")
            || stdout_lower.contains("hats tools memory")
            || stdout_lower.contains("mem-");

        AssertionBuilder::new("Memory command executed")
            .expected("Agent executed hats tools memory add")
            .actual(if executed {
    ...
    fn memory_file_created(&self, exists: bool) -> crate::models::Assertion {
        AssertionBuilder::new("Memory file created")
            .expected(".hats/agent/memories.md file exists")
            .actual(if exists {
...
///
/// This scenario:
/// - Pre-populates `.hats/agent/memories.md` with test data
/// - Uses `hats tools memory search` to find entries
/// - Verifies search results are correct
///
/// # Example
///
/// ```no_run
/// use hats_e2e::scenarios::{MemorySearchScenario, TestScenario};
///
...
impl MemorySearchScenario {
    ...
    pub fn new() -> Self {
        Self {
            ...
            description: "Verifies memories can be searched via hats tools memory search"
                .to_string(),
            tier: "Tier 6: Memory System".to_string(),
        }
    }
...
"#,
        ...
        let prompt = r#"You are testing Hats's memory search functionality.

Pre-existing memories are in .hats/agent/memories.md with these entries:
- A pattern about JWT authentication
- A pattern about database connection pooling
- A fix about docker ECONNREFUSED

Your task:
1. Run `hats tools memory search "database"` to find database-related memories
...
impl MemorySearchScenario {
    ...
    fn search_command_executed(&self, result: &ExecutionResult) -> crate::models::Assertion {
        ...
        AssertionBuilder::new("Search command executed")
            .expected("Agent executed hats tools memory search")
            .actual(if executed {
...
/// This scenario:
/// - Pre-populates `.hats/agent/memories.md` with test data
/// - Configures `inject: auto` in hats.yml
/// - Verifies the agent can see/use the injected memories
///
/// # Example
///
/// ```no_run
/// use hats_e2e::scenarios::{MemoryInjectionScenario, TestScenario};
///
...
### mem-1737400100-a2b2
> This project uses the Hats orchestrator for agentic workflows
...
"#,
        ...
        let prompt = r#"You are testing Hats's memory injection system.
...
///
/// ```no_run
/// use hats_e2e::scenarios::{MemoryPersistenceScenario, TestScenario};
///
...
impl MemoryPersistenceScenario {
    ...
    pub fn new() -> Self {
        Self {
            ...
            description: "Verifies memories persist in .hats/agent/memories.md across runs"
                .to_string(),
            tier: "Tier 6: Memory System".to_string(),
        }
    }
...
"#,
        ...
        // NOTE: The agent needs to use Bash tool to execute the command.
        let prompt = r#"You are testing Hats's memory persistence.

Your task is to add a memory using the Bash tool.

STEP 1: Use the Bash tool to run this exact command:
```
hats tools memory add "Persistence test marker: PERSIST_CHECK_12345" --type context --tags persistence,e2e
```
...
impl MemoryPersistenceScenario {
    ...
    fn memory_persisted_to_disk(&self, exists: bool, content: &str) -> crate::models::Assertion {
        ...
        AssertionBuilder::new("Memory persisted to disk")
            .expected(".hats/agent/memories.md exists with content")
...
///
/// This scenario:
/// - Pre-populates `.hats/agent/memories.md` with malformed content
/// - Verifies Hats doesn't crash when reading corrupted memories
/// - Checks that memory operations still work (add new memories)
///
/// # Example
///
/// ```no_run
/// use hats_e2e::scenarios::{MemoryCorruptedFileScenario, TestScenario};
///
...
1. Try to add a new memory using Bash: hats tools memory add "Chaos test survived" --type context --tags chaos,test
...
impl MemoryCorruptedFileScenario {
    /// Asserts that Hats didn't crash due to corrupted file.
...
///
/// This scenario:
/// - Starts with no `.hats/agent/memories.md` file
...
///
/// ```no_run
/// use hats_e2e::scenarios::{MemoryMissingFileScenario, TestScenario};
///
...
Your task:
1. Add a memory using Bash: hats tools memory add "First memory ever" --type pattern --tags first,test
...
impl MemoryMissingFileScenario {
    /// Asserts that Hats didn't crash due to missing file.
...
///
/// ```no_run
/// use hats_e2e::scenarios::{MemoryRapidWriteScenario, TestScenario};
///
...
1. hats tools memory add "RAPID_TEST_1: First rapid write" --type pattern --tags rapid,test
2. hats tools memory add "RAPID_TEST_2: Second rapid write" --type pattern --tags rapid,test
3. hats tools memory add "RAPID_TEST_3: Third rapid write" --type pattern --tags rapid,test
...
///
/// ```no_run
/// use hats_e2e::scenarios::{MemoryLargeContentScenario, TestScenario};
///
...
hats tools memory add "LARGE_CONTENT_TEST: This is a comprehensive memory entry that contains multiple sentences describing a complex architectural decision. The decision involves choosing between microservices and monolithic architecture for the authentication service. After careful consideration of scalability, maintainability, and team expertise, we decided to implement a modular monolith with clear bounded contexts that can be extracted into microservices later if needed. Key factors: team size (5 engineers), expected load (10K requests/min), and deployment infrastructure (Kubernetes). END_MARKER_12345" --type decision --tags architecture,large,test
...
mod tests {
    ...
    fn test_memory_add_setup_creates_config() {
        ...
        let config_path = workspace.join("hats.yml");
        assert!(config_path.exists(), "hats.yml should exist");
        ...
        assert!(
            workspace.join(".hats").join("agent").exists(),
            ".hats/agent should exist"
        );
    ...
    fn test_memory_search_setup_creates_memories() {
        ...
        let memories_path = workspace.join(".hats/agent/memories.md");
    ...
    async fn test_memory_add_full_run() {
        ...
        let executor = HatsExecutor::new(workspace.clone());

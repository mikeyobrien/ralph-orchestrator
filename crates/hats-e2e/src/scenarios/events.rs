...
//!
//! These scenarios test Hats's event handling capabilities:
//! - Event XML parsing from agent output
//! - Backpressure verification (build.done evidence)
//!
//! Events are the primary communication mechanism between the agent and Hats,
...
/// - Verifies that Hats correctly parses the `<event topic="...">` XML format
/// - Validates that event topics and payloads are captured accurately
///
/// # Example
///
/// ```no_run
/// use hats_e2e::scenarios::{EventsScenario, TestScenario};
///
...
impl TestScenario for EventsScenario {
    ...
    fn setup(&self, workspace: &Path, backend: Backend) -> Result<ScenarioConfig, ScenarioError> {
        ...
        // Create hats.yml for event testing
...
"#,
        ...
        let prompt = r#"You are testing Hats's event parsing system.
...
///
/// Backpressure is Hats's mechanism for ensuring code quality gates are passed
/// before work is considered complete.
///
/// # Example
///
/// ```no_run
/// use hats_e2e::scenarios::{BackpressureScenario, TestScenario};
///
...
impl TestScenario for BackpressureScenario {
    ...
    fn setup(&self, workspace: &Path, backend: Backend) -> Result<ScenarioConfig, ScenarioError> {
        ...
        // Create hats.yml for backpressure testing
        let config_content = format!(
...
"#,
        ...
        let prompt = r#"You are testing Hats's backpressure mechanism.

Backpressure is Hats's quality gate system. You must emit a build.done event.

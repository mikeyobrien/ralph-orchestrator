...
//!
//! This scenario tests Hats's memory + task systems working together across
...
//!
//! This validates:
//! - Memory persistence across Hats invocations
...
/// Test scenario that validates memory + tasks across multiple Hats loops.
///
...
///
/// Each phase runs as a separate Hats invocation, testing:
...
///
/// ```no_run
/// use hats_e2e::scenarios::{IncrementalFeatureScenario, TestScenario};
/// use hats_e2e::Backend;
///
...
impl IncrementalFeatureScenario {
    ...
    pub fn new() -> Self {
        Self {
            ...
            description: "Validates memory + tasks across multiple Hats loops".to_string(),
...
impl TestScenario for IncrementalFeatureScenario {
    ...
    fn setup(&self, workspace: &Path, backend: Backend) -> Result<ScenarioConfig, ScenarioError> {
        ...
        // Create hats.yml with memories + tasks enabled
...
=== PHASE 1: Foundation ===
1. Create a task: `hats tools task add "Create greeting module foundation" -p 1`
2. Write src/greet.py with a simple greet(name) function that returns "Hello, {name}!"
3. Learn the pattern and save it: `hats tools memory add "greet module uses f-string formatting: return f'Hello, {name}!'" --type pattern --tags greet,python`
4. Close the task using its ID
5. Create next task: `hats tools task add "Add error handling to greet" -p 2 --blocked-by <previous-task-id>`
   Actually, since we just closed the previous task, this one is now unblocked.
   Create: `hats tools task add "Add error handling to greet" -p 2`
...
3. Save a memory about the error handling pattern: `hats tools memory add "greet module validates input: raises ValueError for empty name" --type pattern --tags greet,validation`
4. Close the "Add error handling" task
5. Create final task: `hats tools task add "Add docstring to greet" -p 3`
...
4. Verify all tasks are closed: `hats tools task list`
5. Output LOOP_COMPLETE

IMPORTANT RULES:
- Use the Bash tool to execute hats commands
...
Begin with Phase 1."#;
    ...
    async fn run(
        ...
        let tasks_path = executor.workspace().join(".hats/agent/tasks.jsonl");
...
impl IncrementalFeatureScenario {
    ...
    fn tasks_created(&self, exists: bool, content: &str) -> crate::models::Assertion {
        ...
        AssertionBuilder::new("Tasks file created")
            .expected(".hats/agent/tasks.jsonl exists with tasks")
            .actual(if exists && has_content {
...
/// Test scenario that runs multiple sequential Hats invocations.
///
/// Unlike IncrementalFeatureScenario which runs a single long session,
/// this scenario explicitly runs 3 separate Hats invocations to test
/// true cross-session memory and task persistence.
///
/// # Example
///
/// ```no_run
/// use hats_e2e::scenarios::{ChainedLoopScenario, TestScenario};
/// use hats_e2e::Backend;
///
...
impl ChainedLoopScenario {
    ...
    pub fn new() -> Self {
        Self {
            ...
            description: "Tests memory + task persistence across 3 separate Hats invocations"
                .to_string(),
            tier: "Tier 7: Incremental Development".to_string(),
        }
    }
...
Your tasks:
1. Create a task: `hats tools task add "Create calculator module" -p 1`
2. Write src/calc.py with an add(a, b) function that returns a + b
3. Save a pattern memory: `hats tools memory add "Calculator module uses type hints for parameters" --type pattern --tags calc,python`
4. Close the task

Output LOOP_COMPLETE when done.
IMPORTANT: Use Bash for hats commands and Write/Edit for files."#;
...
2. Create a task: `hats tools task add "Add subtract function" -p 2`
3. Add a subtract(a, b) function to src/calc.py
4. Save a memory about the module structure: `hats tools memory add "Calculator module has add and subtract functions" --type context --tags calc,functions`
5. Close the task
...
Your tasks:
1. Read src/calc.py
2. Create a task: `hats tools task add "Add multiply function" -p 3`
3. Add a multiply(a, b) function to src/calc.py
4. Close the task
5. List all tasks to verify completion: `hats tools task list`
6. List all memories to see accumulated knowledge: `hats tools memory list`
...
"#;
    ...
    fn test_chained_loop_setup_creates_structure() {
        ...
        assert!(workspace.join(".agent").exists());
        assert!(workspace.join("src").exists());
        assert!(workspace.join("hats.yml").exists());

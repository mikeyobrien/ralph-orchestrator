...
//!
//! These scenarios test Hats's task tracking system, including:
//! - Adding tasks via CLI
//! - Closing tasks
//! - Loop completion verification with tasks
//! - Dependency tracking
//!
//! Tasks are stored in `.hats/agent/tasks.jsonl` and provide structured
...
///
/// This scenario:
/// - Uses `hats task add` to create a task
/// - Verifies the task is stored in `.hats/agent/tasks.jsonl`
/// - Verifies the task ID format is correct (task-{timestamp}-{hex})
///
/// # Example
///
/// ```no_run
/// use hats_e2e::scenarios::{TaskAddScenario, TestScenario};
...
impl TaskAddScenario {
    ...
    pub fn new() -> Self {
        Self {
            ...
            description: "Verifies tasks can be added via hats task add".to_string(),
...
impl TestScenario for TaskAddScenario {
    ...
    fn setup(&self, workspace: &Path, backend: Backend) -> Result<ScenarioConfig, ScenarioError> {
        ...
        // Create a minimal hats.yml with memories enabled (tasks require memories)
...
impl TaskAddScenario {
    ...
    fn task_command_executed(&self, result: &ExecutionResult) -> crate::models::Assertion {
        ...
        let executed = stdout_lower.contains("task")
            || stdout_lower.contains("hats task")
            || stdout_lower.contains("task-");

        AssertionBuilder::new("Task command executed")
            .expected("Agent executed hats task add")
            .actual(if executed {
    ...
    fn task_file_created(&self, exists: bool) -> crate::models::Assertion {
        AssertionBuilder::new("Task file created")
            .expected(".hats/agent/tasks.jsonl file exists")
            .actual(if exists {
...
/// This scenario:
/// - Creates a task
/// - Closes the task using `hats task close`
/// - Verifies the task status changed to "closed"
///
/// # Example
///
/// ```no_run
/// use hats_e2e::scenarios::{TaskCloseScenario, TestScenario};
/// use hats_e2e::Backend;
///
...
impl TaskCloseScenario {
    ...
    pub fn new() -> Self {
        Self {
            ...
            description: "Verifies tasks can be closed via hats task close".to_string(),
            tier: "Tier 6: Task System".to_string(),
        }
    }
}
...
impl TestScenario for TaskCloseScenario {
    ...
    fn setup(&self, workspace: &Path, backend: Backend) -> Result<ScenarioConfig, ScenarioError> {
        ...
        // Create a minimal hats.yml with memories enabled
        let config_content = format!(
...
"#,
        ...
        let prompt = r#"You are testing Hats's task close functionality.
...
```
hats task add "Task to close" -p 1
```
...
///
/// ```no_run
/// use hats_e2e::scenarios::{TaskCompletionScenario, TestScenario};
/// use hats_e2e::Backend;
///
...
impl TestScenario for TaskCompletionScenario {
    ...
    fn setup(&self, workspace: &Path, backend: Backend) -> Result<ScenarioConfig, ScenarioError> {
        ...
        // Create hats.yml with memories enabled
        let config_content = format!(
...
/// Test scenario that verifies `hats task ready` only shows unblocked tasks.
///
/// This scenario:
/// - Creates two tasks, one blocked by the other
/// - Verifies `hats task ready` only shows the unblocked task
///
/// # Example
///
/// ```no_run
/// use hats_e2e::scenarios::{TaskReadyScenario, TestScenario};
/// use hats_e2e::Backend;
///
...
impl TaskReadyScenario {
    ...
    pub fn new() -> Self {
        Self {
            ...
            description: "Verifies hats task ready shows only unblocked tasks".to_string(),
            tier: "Tier 6: Task System".to_string(),
        }
    }
}
...
"#,
        ...
        let prompt = r#"You are testing Hats's task dependency system.

STEP 1: Create a parent task:
```
hats task add "Parent task" -p 1
```
Note the task ID from the output.

STEP 2: Create a child task blocked by the parent (use the parent's task ID):
```
hats task add "Child task" -p 2 --blocked-by <parent-task-id>
```

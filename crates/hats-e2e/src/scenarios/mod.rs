...
//! │                      TestScenario                               │
//! ├─────────────────────────────────────────────────────────────────┤
//! │  1. setup()   → Creates hats.yml, prompt files in workspace    │
//! │  2. run()     → Executes via HatsExecutor, checks assertions   │
...
//!
//! ```no_run
//! use hats_e2e::scenarios::{TestScenario, ConnectivityScenario};
...
//!     let executor = HatsExecutor::new(workspace.to_path_buf());
...
use crate::Backend;
use crate::executor::{ExecutionResult, HatsExecutor, ScenarioConfig};
...
/// - Execute against a backend via HatsExecutor
...
impl Assertions {
    ...
    /// Hats completed via the completion promise or hit iteration limits.

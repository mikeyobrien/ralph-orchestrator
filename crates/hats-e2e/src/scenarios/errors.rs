...
//!
//! These scenarios test Hats's graceful failure modes:
//! - Timeout handling
...
/// - Verifies that Hats terminates gracefully at timeout
/// - Validates that the timeout flag is set correctly
///
/// # Example
///
/// ```no_run
/// use hats_e2e::scenarios::{TimeoutScenario, TestScenario};
///
...
impl TestScenario for TimeoutScenario {
    ...
    fn setup(&self, workspace: &Path, backend: Backend) -> Result<ScenarioConfig, ScenarioError> {
        ...
        // Create hats.yml for timeout testing
...
/// - Verifies that Hats terminates at the iteration limit
///
/// # Example
///
/// ```no_run
/// use hats_e2e::scenarios::{MaxIterationsScenario, TestScenario};
///
...
impl TestScenario for MaxIterationsScenario {
    ...
    fn setup(&self, workspace: &Path, backend: Backend) -> Result<ScenarioConfig, ScenarioError> {
        ...
        // Create hats.yml with low max iterations
        let config_content = format!(
...
/// - Verifies that Hats handles auth failures gracefully
...
///
/// ```no_run
/// use hats_e2e::scenarios::{AuthFailureScenario, TestScenario};
///
...
impl TestScenario for AuthFailureScenario {
    ...
    fn setup(&self, workspace: &Path, backend: Backend) -> Result<ScenarioConfig, ScenarioError> {
        ...
        // Create hats.yml - we'll pass bad auth via environment
        let config_content = format!(
...
/// - Verifies that Hats handles missing backends gracefully
/// - Validates error messages guide the user
///
/// # Example
///
/// ```no_run
/// use hats_e2e::scenarios::{BackendUnavailableScenario, TestScenario};
///
...
impl TestScenario for BackendUnavailableScenario {
    ...
    fn setup(&self, workspace: &Path, backend: Backend) -> Result<ScenarioConfig, ScenarioError> {
        ...
        // Create hats.yml with a nonexistent backend command
        let config_content = format!(

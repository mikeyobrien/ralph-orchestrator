//! Integration tests for `hats memory` CLI commands.
//!
//! Tests the memory subcommands per the hats-tools skill specification.
...
/// Run hats tools memory command with given args in the temp directory.
fn hats_memory(temp_path: &std::path::Path, args: &[&str]) -> std::process::Output {
    Command::new(env!("CARGO_BIN_EXE_hats"))
        ...
        .output()
        .expect("Failed to execute hats command")
}

/// Run hats tools memory command and assert success.
fn hats_memory_ok(temp_path: &std::path::Path, args: &[&str]) -> String {
    let output = hats_memory(temp_path, args);
    assert!(
        output.status.success(),
        "Command 'hats tools memory {}' failed: {}",
        args.join(" "),
...
fn test_memory_init_creates_file() -> Result<()> {
    ...
    // Ensure .hats/agent directory and memories.md don't exist
    let memories_path = temp_path.join(".hats/agent/memories.md");
    assert!(!memories_path.exists());

    // Run init
    let stdout = hats_memory_ok(temp_path, &["init"]);
...
fn test_memory_init_fails_without_force() -> Result<()> {
    ...
    let agent_dir = temp_path.join(".hats/agent");
    fs::create_dir_all(&agent_dir)?;
    fs::write(agent_dir.join("memories.md"), "# Existing content")?;

    // Run init without --force
    let output = hats_memory(temp_path, &["init"]);
...
fn test_memory_init_force_overwrites() -> Result<()> {
    ...
    // Run init with --force
    hats_memory_ok(temp_path, &["init", "--force"]);
...
fn test_memory_add_creates_file() -> Result<()> {
    ...
    let stdout = hats_memory_ok(temp_path, &["add", "test memory content"]);
...
fn test_memory_add_with_type() -> Result<()> {
    ...
    let content = fs::read_to_string(temp_path.join(".hats/agent/memories.md"))?;
    assert!(content.contains("## Fixes"));
...
fn test_memory_add_quiet_format() -> Result<()> {
    ...
    let stdout = hats_memory_ok(temp_path, &["add", "quiet test", "--format", "quiet"]);
...
fn test_memory_add_json_format() -> Result<()> {
    ...
    let stdout = hats_memory_ok(temp_path, &["add", "json test", "--format", "json"]);
...
fn test_memory_list_shows_all() -> Result<()> {
    ...
    hats_memory_ok(temp_path, &["add", "first memory"]);
    hats_memory_ok(temp_path, &["add", "second memory", "-t", "fix"]);
    hats_memory_ok(temp_path, &["add", "third memory", "-t", "decision"]);

    // List all
    let stdout = hats_memory_ok(temp_path, &["list"]);
...
fn test_memory_list_filter_by_type() -> Result<()> {
    ...
    hats_memory_ok(temp_path, &["add", "pattern one"]);
    hats_memory_ok(temp_path, &["add", "fix one", "-t", "fix"]);
    hats_memory_ok(temp_path, &["add", "fix two", "-t", "fix"]);

    // List only fixes
    let stdout = hats_memory_ok(temp_path, &["list", "-t", "fix"]);
...
fn test_memory_list_last_n() -> Result<()> {
    ...
    // Add multiple memories
    hats_memory_ok(temp_path, &["add", "mem1"]);
    hats_memory_ok(temp_path, &["add", "mem2"]);
    hats_memory_ok(temp_path, &["add", "mem3"]);
    hats_memory_ok(temp_path, &["add", "mem4"]);

    // List only last 2
    let stdout = hats_memory_ok(temp_path, &["list", "--last", "2"]);
...
fn test_memory_list_json_format() -> Result<()> {
    ...
    hats_memory_ok(temp_path, &["add", "test memory"]);

    let stdout = hats_memory_ok(temp_path, &["list", "--format", "json"]);
...
fn test_memory_show_by_id() -> Result<()> {
    ...
    // Add and capture ID
    let add_stdout = hats_memory_ok(temp_path, &["add", "show test memory", "--format", "quiet"]);
    let memory_id = add_stdout.trim();

    // Show by ID
    let stdout = hats_memory_ok(temp_path, &["show", memory_id]);
...
fn test_memory_show_not_found() -> Result<()> {
    ...
    let output = hats_memory(temp_path, &["show", "mem-0000000000-xxxx"]);
...
fn test_memory_show_json_format() -> Result<()> {
    ...
    let add_stdout = hats_memory_ok(temp_path, &["add", "json show test", "--format", "quiet"]);
    let memory_id = add_stdout.trim();

    let stdout = hats_memory_ok(temp_path, &["show", memory_id, "--format", "json"]);
...
fn test_memory_delete_removes_entry() -> Result<()> {
    ...
    // Add a memory
    let add_stdout = hats_memory_ok(temp_path, &["add", "to be deleted", "--format", "quiet"]);
    let memory_id = add_stdout.trim();

    // Verify it exists
    let content_before = fs::read_to_string(temp_path.join(".hats/agent/memories.md"))?;
    assert!(content_before.contains("to be deleted"));

    // Delete it
    let stdout = hats_memory_ok(temp_path, &["delete", memory_id]);
    assert!(
    ...
    // Verify it's gone
    let content_after = fs::read_to_string(temp_path.join(".hats/agent/memories.md"))?;
...
fn test_memory_delete_not_found() -> Result<()> {
    ...
    let output = hats_memory(temp_path, &["delete", "mem-0000000000-xxxx"]);
...
fn test_memory_search_finds_by_content() -> Result<()> {
    ...
    hats_memory_ok(temp_path, &["add", "uses barrel exports everywhere"]);
    hats_memory_ok(temp_path, &["add", "API routes use kebab-case"]);
    hats_memory_ok(temp_path, &["add", "chose Postgres over SQLite"]);

    // Search for "barrel"
    let stdout = hats_memory_ok(temp_path, &["search", "barrel"]);
...
fn test_memory_search_finds_by_tags() -> Result<()> {
    ...
    hats_memory_ok(temp_path, &["add", "docker compose up", "--tags", "docker"]);

    // Search by tag
    let stdout = hats_memory_ok(temp_path, &["search", "--tags", "docker"]);
...
fn test_memory_search_filter_by_type() -> Result<()> {
    ...
    hats_memory_ok(temp_path, &["add", "a pattern", "-t", "pattern"]);
    hats_memory_ok(temp_path, &["add", "a fix", "-t", "fix"]);
    hats_memory_ok(temp_path, &["add", "another fix", "-t", "fix"]);

    // Search only fixes
    let stdout = hats_memory_ok(temp_path, &["search", "-t", "fix"]);
...
fn test_memory_search_no_results() -> Result<()> {
    ...
    hats_memory_ok(temp_path, &["add", "something else entirely"]);

    let stdout = hats_memory_ok(temp_path, &["search", "nonexistent_xyz"]);
...
fn test_memory_search_json_format() -> Result<()> {
    ...
    hats_memory_ok(temp_path, &["add", "searchable memory"]);

    let stdout = hats_memory_ok(temp_path, &["search", "searchable", "--format", "json"]);
...
fn test_memory_prime_outputs_markdown() -> Result<()> {
    ...
    hats_memory_ok(temp_path, &["add", "prime test one"]);
    hats_memory_ok(temp_path, &["add", "prime test two", "-t", "fix"]);

    let stdout = hats_memory_ok(temp_path, &["prime"]);
...
fn test_memory_prime_respects_budget() -> Result<()> {
    ...
    let stdout = hats_memory_ok(temp_path, &["prime", "--budget", "100"]);
...
fn test_memory_prime_filter_by_type() -> Result<()> {
    ...
    hats_memory_ok(temp_path, &["add", "pattern memory"]);
    hats_memory_ok(temp_path, &["add", "fix memory", "-t", "fix"]);

    // Prime only patterns
    let stdout = hats_memory_ok(temp_path, &["prime", "-t", "pattern"]);
...
fn test_memory_prime_filter_by_tags() -> Result<()> {
    ...
    hats_memory_ok(temp_path, &["add", "docker memory", "--tags", "docker"]);
    hats_memory_ok(temp_path, &["add", "nginx memory", "--tags", "nginx"]);

    // Prime only docker tag
    let stdout = hats_memory_ok(temp_path, &["prime", "--tags", "docker"]);
...
fn test_memory_prime_json_format() -> Result<()> {
    ...
    hats_memory_ok(temp_path, &["add", "json prime test"]);

    let stdout = hats_memory_ok(temp_path, &["prime", "--format", "json"]);
...
fn test_memory_list_color_never() -> Result<()> {
    ...
    hats_memory_ok(temp_path, &["add", "color test"]);

    // Run with --color never via the main CLI
    let output = Command::new(env!("CARGO_BIN_EXE_hats"))
        .arg("--color")
        .arg("never")
        .arg("tools")
        .arg("memory")

...
///
/// Tests the `hats clean` command which removes Hats-generated artifacts
/// from the `.hats/agent/` directory.
...
"#;
    ...
    // Create .hats/agent directory with files
    ...
    assert!(
        !agent_dir.exists(),
        ".hats/agent directory should be deleted"
    );
...
"#;
    ...
    // Run hats clean with custom config
    let output = Command::new(env!("CARGO_BIN_EXE_hats"))
        .arg("clean")
        .arg("--config")
...
"#;
    ...
    assert!(
        agent_dir.exists(),
        ".hats/agent directory should still exist after dry-run"
    );
    assert!(agent_dir.join("scratchpad.md").exists());
...
"#;
    ...
    // Don't create .hats/agent directory
    let agent_dir = temp_path.join(".hats/agent");
    assert!(!agent_dir.exists());
...
fn test_clean_color_output_never() -> Result<()> {
    ...
    // Create config and .hats/agent directory
    let config_content = r#"
core:
  scratchpad: ".hats/agent/scratchpad.md"
"#;
    ...
    // Run hats clean with --color never
    let output = Command::new(env!("CARGO_BIN_EXE_hats"))
        .arg("clean")
        .arg("--config")
        .arg(temp_path.join("hats.yml"))
...
"#;
    ...
    // Run hats clean with --color always
    let output = Command::new(env!("CARGO_BIN_EXE_hats"))
        .arg("clean")
        .arg("--config")
        .arg(temp_path.join("hats.yml"))

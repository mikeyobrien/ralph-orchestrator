//! Integration tests for `hats tools skill` CLI commands.

use std::process::Command;
use std::{fs, path::Path};
use tempfile::TempDir;

fn hats_skill(temp_path: &std::path::Path, args: &[&str]) -> std::process::Output {
    Command::new(env!("CARGO_BIN_EXE_hats"))
        ...
        .output()
        .expect("Failed to execute hats tools skill command")
}

fn hats_skill_no_root(current_path: &std::path::Path, args: &[&str]) -> std::process::Output {
    Command::new(env!("CARGO_BIN_EXE_hats"))
        .arg("tools")
        .arg("skill")
        .args(args)
...
fn hats_skill_ok(temp_path: &std::path::Path, args: &[&str]) -> String {
    let output = hats_skill(temp_path, args);
    assert!(
        output.status.success(),
        "Command 'hats tools skill {}' failed: {}",
        args.join(" "),
...
fn hats_skill_no_root_ok(current_path: &std::path::Path, args: &[&str]) -> String {
    let output = hats_skill_no_root(current_path, args);
    assert!(
        output.status.success(),
        "Command 'hats tools skill {}' failed: {}",
        args.join(" "),
...
fn test_skill_load_builtin() {
    ...
    let stdout = hats_skill_ok(temp_path, &["load", "hats-tools"]);
    assert!(stdout.contains("Hats Tools"));
    assert!(stdout.contains("hats tools task"));
}
...
fn test_skill_load_missing_exits_nonzero() {
    ...
    let output = hats_skill(temp_path, &["load", "missing-skill"]);
...
fn test_skill_list_includes_builtins() {
    ...
    let stdout = hats_skill_ok(temp_path, &["list", "--format", "quiet"]);
    let lines: Vec<&str> = stdout.lines().collect();
    assert!(lines.contains(&"hats-tools"));
...
",
    ...
    let list_stdout = hats_skill_ok(temp_path, &["list", "--format", "quiet"]);
    let list_lines: Vec<&str> = list_stdout.lines().collect();
    assert!(list_lines.contains(&"test-driven-development"));

    let load_stdout = hats_skill_ok(temp_path, &["load", "test-driven-development"]);
...
fn test_skill_load_finds_nested_skills_dir_when_root_missing() {
    ...
    fs::write(temp_path.join("hats.yml"), "skills:\n  enabled: true\n").expect("write hats.yml");
...
",
    ...
    let list_stdout = hats_skill_no_root_ok(&nested_dir, &["list", "--format", "quiet"]);
    let list_lines: Vec<&str> = list_stdout.lines().collect();
    assert!(list_lines.contains(&"test-driven-development"));

    let load_stdout = hats_skill_no_root_ok(&nested_dir, &["load", "test-driven-development"]);
...
fn test_skill_load_finds_parent_skills_dir_when_root_nested() {
    ...
    let repo_dir = temp_path.join("repo");
    let workspace_dir = repo_dir.join("hats");
    let nested_dir = workspace_dir.join("nested");
    fs::create_dir_all(&nested_dir).expect("create nested dir");

    fs::write(
        workspace_dir.join("hats.yml"),
        "skills:\n  enabled: true\n",
    )
    .expect("write hats.yml");

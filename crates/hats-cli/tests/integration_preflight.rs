//! Integration tests for `hats preflight` CLI command.

use std::process::Command;
use tempfile::TempDir;

fn hats_preflight(temp_path: &std::path::Path, args: &[&str]) -> std::process::Output {
    Command::new(env!("CARGO_BIN_EXE_hats"))
        .args(args)
        .current_dir(temp_path)
        .output()
        .expect("Failed to execute hats preflight command")
}

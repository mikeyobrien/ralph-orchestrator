...
mod web_integration {
    ...
    fn test_web_command_with_fake_node_tools() {
        ...
        fs::create_dir_all(workspace.join("backend/hats-web-server")).expect("backend dir");
        fs::create_dir_all(workspace.join("frontend/hats-web")).expect("frontend dir");
...
exit 1",
        ...
        assert!(stdout.contains("Starting Hats web servers"));
    }
}

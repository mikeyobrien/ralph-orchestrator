...
- **Planning only:** This SOP produces planning artifacts. You MUST NOT implement code, run containers, execute scripts, or begin any implementation work. If the user wants implementation, direct them to `hats run`.

## Parameters

- **rough_idea** (required): The initial concept or idea to develop
- **project_dir** (optional, default: `specs/{task_name}/`): Base directory for all artifacts. `{task_name}` is derived as kebab-case from the idea (e.g., "build a rate limiter" → `rate-limiter`). Aligns with Hats's spec-driven pipeline.
...
Inform the user the structure feeds into Hats's spec-driven presets.
...
### 9. Offer Hats Integration

Ask: "Would you like me to create a PROMPT.md for Hats to implement this autonomously?"
...
- Full pipeline: `hats run --config presets/pdd-to-code-assist.yml`
- Simpler flow: `hats run --config presets/spec-driven.yml`

If the user declines, acknowledge and conclude the session.

**Gate:** You MUST NOT run `hats run` or begin any implementation. This SOP ends here. Implementation is a separate step the user initiates themselves.

...
//! the Hats orchestration loop, along with supporting types and helper
...
use hats_core::{
    ...
    LoopContext, LoopHistory, LoopRegistry, MergeQueue, HatsConfig, Record, SessionRecorder,
...
pub async fn run_loop_impl(
    ...
    // This file is read by `hats tools task add` to tag new tasks.
    ...
    let loop_id_marker = ctx.hats_dir().join("current-loop-id");
    ...
    // The marker file `.hats/current-events` coordinates path between Hats and agents
    if !resume {
        let run_id = chrono::Utc::now().format("%Y%m%d-%H%M%S").to_string();
        // Use relative path in marker file for portability across agents
        // The actual file is at ctx.hats_dir()/events-{run_id}.jsonl
        let relative_events_path = format!(".hats/events-{}.jsonl", run_id);

        fs::create_dir_all(ctx.hats_dir()).context("Failed to create .hats directory")?;
    ...
    // Append custom args from CLI if provided (e.g., `hats run -b opencode -- --model="some-model"`)
    ...
    let (mut tui_handle, tui_state) = if enable_tui {
        ...
        // instead of generic "hats" for all events
    ...
    if let Some(ref loop_id) = merge_loop_id {
        ...
        match queue.mark_merging(loop_id, pid) {
            ...
            Err(hats_core::MergeQueueError::NotFound(_)) => {
                warn!(loop_id = %loop_id, "Merge loop started but no queue entry found");
            }
            Err(hats_core::MergeQueueError::InvalidTransition(_, from, _)) => {
    ...
    loop {
        ...
        // When Hats is coordinating (hat_id == "hats"), show the active hat being worked on
        let display_hat = if hat_id.as_str() == "hats" {
        ...
        // Use display_hat (the active hat) instead of hat_id ("hats" in multi-hat mode)
                    ...
                    match CliBackend::from_hat_backend(hat_backend) {
                        Ok(hat_backend_instance) => {
                            ...
                            let backend_name = match hat_backend {
                                hats_core::HatBackend::Named(name) => name.clone(),
                                hats_core::HatBackend::NamedWithArgs { backend_type, .. } => {
                                    backend_type.clone()
                                }
                                hats_core::HatBackend::KiroAgent { .. } => "kiro".to_string(),
                                ...
                                hats_core::HatBackend::Custom { command, .. } => {
...
fn convert_termination_type(
    termination_type: hats_adapters::TerminationType,
    interactive: bool,
) -> Option<TerminationReason> {
    match termination_type {
        hats_adapters::TerminationType::Natural => None,
        hats_adapters::TerminationType::IdleTimeout => {
            if interactive {
        ...
        hats_adapters::TerminationType::UserInterrupt
        | hats_adapters::TerminationType::ForceKill => Some(TerminationReason::Interrupted),
    }
}

fn prepare_tui_iteration(
    tui_state: &Arc<std::sync::Mutex<hats_tui::TuiState>>,
    hat_display: String,
    backend: String,
    max_iterations: u32,
...
/// system event to help Hats understand the misconfiguration.
fn log_events_from_output(
    logger: &mut EventLogger,
    iteration: u32,
    hat_id: &HatId,
    output: &str,
    registry: &hats_core::HatRegistry,
) {
    ...
    for event in events {
        ...
        if let Some(triggered_hat) = triggered {
            ...
            // Emit event.orphaned system event so Hats sees the problem
...
fn resolve_prompt_content(event_loop_config: &hats_core::EventLoopConfig) -> Result<String> {
    debug!(
...
///
/// When running in planning mode (HATS_PLANNING_SESSION_ID is set),
...
fn check_planning_session_responses_for_session(
    ...
    for line in conversation_content.lines() {
        ...
        let entry: hats_core::planning_session::ConversationEntry =
        ...
        if entry.entry_type != hats_core::planning_session::ConversationType::UserResponse {
            continue;
        }
...
///
/// Called when the primary loop completes successfully. Spawns merge-hats
/// processes for each queued loop in FIFO order.
fn process_pending_merges_with_command(repo_root: &Path, hats_cmd: &OsStr) {
    let queue = MergeQueue::new(repo_root);

    // Get all pending merges
    let pending = match queue.list_by_state(hats_core::merge_queue::MergeState::Queued) {
        Ok(entries) => entries,
        Err(e) => {
    ...
    let config_path = repo_root.join(".hats/merge-loop-config.yml");
    ...
    for entry in pending {
        ...
        info!(loop_id = %loop_id, "Spawning merge-hats process");

        match Command::new(hats_cmd)
            .current_dir(repo_root)
            .args([
                "run",
                "-c",
                ".hats/merge-loop-config.yml",
                "--exclusive",
                "--no-tui",
                "-p",
                &format!("Merge loop {} from branch hats/{}", loop_id, loop_id),
            ])
            .env("HATS_MERGE_LOOP_ID", loop_id)
            .spawn()
        {
            Ok(child) => {
                info!(
                    loop_id = %loop_id,
                    pid = child.id(),
                    "merge-hats spawned successfully"
                );
            }
            Err(e) => {
                warn!(
                    loop_id = %loop_id,
                    error = %e,
                    "Failed to spawn merge-hats, loop will remain queued for manual retry"
                );
            }
        }
    }
...
fn process_pending_merges(repo_root: &Path) {
    process_pending_merges_with_command(repo_root, OsStr::new("hats"));
}

/// Public wrapper for CLI invocation of process_pending_merges.
///
/// Called by `hats loops process` command to process the merge queue.
...
///
/// Loads config from `hats.yml`, applies the given prompt, acquires the
...
pub async fn start_loop(
    ...
    let config_source = config_path.unwrap_or_else(|| workspace_root.join("hats.yml"));
    ...
    if config.cli.backend == "auto" {
        ...
        let detected = hats_adapters::detect_backend(&priority, |backend| {
    ...
    let prompt_summary = hats_core::truncate_with_ellipsis(prompt_summary, 100);

    let _lock_guard = hats_core::LoopLock::try_acquire(&workspace_root, &prompt_summary)
        .context("Failed to acquire loop lock — another loop may be running")?;

    let loop_context = hats_core::LoopContext::primary(workspace_root);
...
fn create_robot_service(
    config: &HatsConfig,
    context: &LoopContext,
) -> Option<Box<dyn hats_proto::RobotService>> {
    ...
    match hats_telegram::TelegramService::new(workspace_root, bot_token, timeout_secs, loop_id) {
        Ok(service) => {
...
mod tests {
    ...
    use hats_core::HatRegistry;
    use hats_core::planning_session::{ConversationEntry, ConversationType};
    use hats_proto::{Hat, Topic};
    use std::ffi::OsStr;
    use std::sync::Arc;
    use std::sync::Mutex;
    ...
    fn test_prepare_tui_iteration_seeds_max_iterations() {
        let state = Arc::new(Mutex::new(hats_tui::TuiState::new()));
    ...
    fn test_idle_timeout_interactive_mode_continues() {
        ...
        let termination_type = hats_adapters::TerminationType::IdleTimeout;
    ...
    fn test_natural_termination_always_continues() {
        ...
        let termination_type = hats_adapters::TerminationType::Natural;
    ...
    fn test_user_interrupt_always_terminates() {
        ...
        let termination_type = hats_adapters::TerminationType::UserInterrupt;
    ...
    fn test_force_kill_always_terminates() {
        ...
        let termination_type = hats_adapters::TerminationType::ForceKill;
    ...
    fn test_process_pending_merges_handles_missing_preset() {
        let temp_dir = tempfile::tempdir().expect("temp dir");
        let repo_root = temp_dir.path();
        std::fs::create_dir_all(repo_root.join(".hats/merge-queue")).expect("queue dir");
    ...
    fn test_process_pending_merges_spawns_for_queue_entry() {
        ...
        let queue_file = repo_root.join(".hats/merge-queue/loop-1234.json");
        std::fs::write(
            &queue_file,
        ...
        let hats_path = write_fake_executable(&bin_dir, "hats", "exit 0");

        process_pending_merges_with_command(repo_root, hats_path.as_os_str());
    }
    ...
    fn test_process_pending_merges_missing_command_keeps_queue() {
        let temp_dir = tempfile::tempdir().expect("temp dir");
        let repo_root = temp_dir.path();
        let queue = hats_core::merge_queue::MergeQueue::new(repo_root);
        queue.enqueue("loop-9999", "merge prompt").expect("enqueue");

        process_pending_merges_with_command(repo_root, OsStr::new("hats-command-missing-12345"));

        let config_path = repo_root.join(".hats/merge-loop-config.yml");
        assert!(config_path.exists());
        let entries = queue
            .list_by_state(hats_core::merge_queue::MergeState::Queued)
            .expect("list queued");
...
<event topic=\"unknown.event\">oops</event>";
    ...
    fn test_check_planning_session_responses_publishes_user_response() {
        ...
        let ctx = hats_core::LoopContext::primary(temp_dir.path().to_path_buf());

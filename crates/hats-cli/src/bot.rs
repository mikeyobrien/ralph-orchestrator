...
//!
//! Provides:
//! - `hats bot onboard --telegram` — Interactive wizard for Telegram bot setup
//! - `hats bot status` — Check current bot configuration status
//! - `hats bot test` — Send a test message to verify the bot works
//! - `hats bot token set <token>` — Store/overwrite the bot token
...
#[derive(Parser, Debug)]
pub struct TestArgs {
    /// Message to send (default: "Hello from Hats!")
    #[arg(default_value = "Hello from Hats!")]
    pub message: String,
}
...
fn bot_token_set(args: SetTokenArgs, use_colors: bool) -> Result<()> {
    ...
    match store_bot_token(&token) {
        Ok(()) => {
            ...
            print_success(
                use_colors,
                "Token stored in OS keychain (hats/telegram-bot-token)",
            );
        }
        Err(e) => {
            print_warning(
    ...
    let config_path = args
        .config
        .clone()
        .unwrap_or_else(|| PathBuf::from("hats.yml"));
    ...
    if !keychain_ok && !has_config {
        print_warning(
            use_colors,
            "Keychain storage failed; token saved to hats.yml instead.",
        );
    }
...
async fn onboard_telegram(args: OnboardArgs, use_colors: bool) -> Result<()> {
    println!();
    if use_colors {
        println!("\x1b[1mHats Telegram Bot Setup\x1b[0m");
        println!("\x1b[1m========================\x1b[0m");
    } else {
        println!("Hats Telegram Bot Setup");
        println!("========================");
    }
    println!();
    ...
    let chat_id = if let Some(id) = args.chat_id {
        ...
        match telegram_get_updates(&token, args.timeout).await {
            ...
            Err(e) => {
                ...
                println!(
                    "    - You can retry with: hats bot onboard --token <token> --timeout 300"
                );
    ...
    match store_bot_token(&token) {
        ...
        Err(e) => {
            ...
            println!("    Set HATS_TELEGRAM_BOT_TOKEN env var instead.");
    ...
    match save_robot_config(args.timeout, config_token) {
        Ok(()) => {
            if config_token.is_some() {
                print_warning(
                    use_colors,
                    "Stored bot token in hats.yml (legacy). Consider using env var or keychain.",
                );
            }
            print_success(use_colors, "Updated hats.yml (RObot.enabled: true)");
        }
        Err(e) => {
            print_warning(use_colors, &format!("Could not update hats.yml: {e}"));
    ...
    match save_telegram_state(chat_id) {
        Ok(()) => {
            print_success(
                use_colors,
                &format!("Created .hats/telegram-state.json (chat_id: {})", chat_id),
            );
        }
        Err(e) => {
    ...
    match telegram_send_message(
        &token,
        chat_id,
        "Hats bot setup complete! I'm ready to assist during orchestration runs.",
    )
    .await
    {
        Ok(_) => {
        ...
        Err(e) => {
            ...
            println!("    Setup saved. Verify later with: hats bot test");
        }
    }
    ...
    if use_colors {
        println!(
            "\x1b[32mSetup complete!\x1b[0m Run `hats run` to start with Telegram integration."
        );
    } else {
        println!("Setup complete! Run `hats run` to start with Telegram integration.");
    }
...
async fn bot_status(use_colors: bool) -> Result<()> {
    println!();
    if use_colors {
        println!("\x1b[1mHats Bot Status\x1b[0m");
        println!("\x1b[1m================\x1b[0m");
    } else {
        println!("Hats Bot Status");
        println!("================");
    }
    println!();
    ...
    // Check env var
    let has_env = std::env::var("HATS_TELEGRAM_BOT_TOKEN").is_ok();
    if has_env {
        print_success(use_colors, "Env var: HATS_TELEGRAM_BOT_TOKEN set");
    } else {
        print_status(use_colors, "Env var: HATS_TELEGRAM_BOT_TOKEN not set");
    }
    ...
    if config_token.is_some() {
        print_warning(
            use_colors,
            "Config: bot_token in hats.yml (consider migrating to keychain)",
        );
    } else {
        print_status(use_colors, "Config: no token in hats.yml");
    }

    // Check RObot enabled
    let robot_enabled = is_robot_enabled();
    if robot_enabled {
        print_success(use_colors, "RObot: enabled in hats.yml");
    } else {
        print_status(use_colors, "RObot: not enabled in hats.yml");
    }

    // Check telegram state
    let state_path = Path::new(".hats/telegram-state.json");
    ...
    let effective_token = std::env::var("HATS_TELEGRAM_BOT_TOKEN")
        .ok()
        .or(keychain_token)
        .or(config_token);
    ...
    if let Some(token) = effective_token {
        ...
        print_error(
            use_colors,
            "No token available. Run `hats bot onboard --telegram` to set up.",
        );
    }
...
async fn bot_test(args: TestArgs, use_colors: bool) -> Result<()> {
    ...
    let token = resolve_token().context(
        "No bot token available. Run `hats bot onboard --telegram` or set HATS_TELEGRAM_BOT_TOKEN",
    )?;

    // Resolve chat_id
    let chat_id = resolve_chat_id()
        .context("No chat_id found. Run `hats bot onboard --telegram` to detect it")?;
...
async fn run_daemon(
    ...
    let config_path = match primary_sources.first() {
        ...
        Some(ConfigSource::Builtin(_)) => {
            anyhow::bail!(
                "Builtin presets are not supported for `hats bot daemon`. Use a file path via -c/--config."
            );
        }
        Some(ConfigSource::Remote(_)) => {
            anyhow::bail!(
                "Remote config URLs are not supported for `hats bot daemon`. Use a file path via -c/--config."
            );
        }
        Some(ConfigSource::Override { .. }) => unreachable!("Partitioned out overrides"),
        None => Some(workspace_root.join("hats.yml")),
    };
    ...
    if use_colors {
        println!("\x1b[1mHats Daemon\x1b[0m (Telegram)");
    } else {
        println!("Hats Daemon (Telegram)");
    }

    // Build the adapter
    let adapter = hats_telegram::TelegramDaemon::new(token, chat_id);

    // Build the start_loop callback — wraps our CLI loop runner
    let start_loop: hats_proto::StartLoopFn = Box::new(move |prompt: String| {
...
fn store_bot_token(token: &str) -> Result<()> {
    let entry = keyring::Entry::new("hats", "telegram-bot-token")
...
fn load_bot_token() -> Option<String> {
    keyring::Entry::new("hats", "telegram-bot-token")
        .ok()
...
/// Save RObot config to hats.yml.
///
/// If hats.yml exists, parses it and updates the RObot section.
/// If it doesn't exist, creates a minimal config.
fn save_robot_config(timeout: u64, bot_token: Option<&str>) -> Result<()> {
    let config_path = Path::new("hats.yml");
    ...
    if config_path.exists() {
        ...
        let content = std::fs::read_to_string(config_path).context("Failed to read hats.yml")?;

        let mut doc: serde_yaml::Value =
            serde_yaml::from_str(&content).context("Failed to parse hats.yml")?;
        ...
        std::fs::write(config_path, yaml_str).context("Failed to write hats.yml")?;
    } else {
        ...
        std::fs::write(config_path, yaml).context("Failed to create hats.yml")?;
    }
...
fn save_telegram_state(chat_id: i64) -> Result<()> {
    let state_dir = Path::new(".hats");
    if !state_dir.exists() {
        std::fs::create_dir_all(state_dir).context("Failed to create .hats directory")?;
    }
...
/// Read bot token from hats.yml (legacy).
fn load_config_bot_token() -> Option<String> {
    load_config_bot_token_from(Path::new("hats.yml"))
}

/// Check if RObot is enabled in config.
fn is_robot_enabled() -> bool {
    let content = match std::fs::read_to_string("hats.yml") {
        Ok(c) => c,
        Err(_) => return false,
    };
...
pub(crate) fn resolve_token() -> Option<String> {
    resolve_token_from(
        std::env::var("HATS_TELEGRAM_BOT_TOKEN").ok(),
        load_bot_token(),
...
pub(crate) fn resolve_chat_id() -> Option<i64> {
    let content = std::fs::read_to_string(".hats/telegram-state.json").ok()?;
...
mod tests {
    ...
    async fn test_run_daemon_rejects_remote_config() {
        let sources = vec![ConfigSource::Remote(
            "https://example.com/hats.yml".to_string(),
        )];
        let err = run_daemon(DaemonArgs {}, &sources, false)
            .await
    ...
    fn test_save_telegram_state_creates_file() {
        ...
        let state_path = temp_dir.path().join(".hats").join("telegram-state.json");
    ...
    fn test_save_robot_config_creates_minimal_config_without_token() {
        ...
        let content = std::fs::read_to_string("hats.yml").unwrap();
    ...
    fn test_telegram_get_me_parses_response() {
        ...
        assert_eq!(first_name, "Hats Bot");
        assert_eq!(username, "hats_test_bot");
    }
    ...
    fn test_telegram_send_message_payload() {
        ...
        let payload = serde_json::json!({
            ...
            "text": "Hello from Hats!",
        });

        assert_eq!(payload["chat_id"].as_i64().unwrap(), 123_456_789);
        assert_eq!(payload["text"].as_str().unwrap(), "Hello from Hats!");
    }
    ...
    fn test_is_robot_enabled_invalid_yaml_returns_false() {
        let temp_dir = tempfile::tempdir().unwrap();
        let _cwd = CwdGuard::set(temp_dir.path());
        std::fs::write(temp_dir.path().join("hats.yml"), "not: [valid").unwrap();
    ...
    fn test_resolve_chat_id_invalid_json_returns_none() {
        ...
        let _cwd = CwdGuard::set(temp_dir.path());
        std::fs::create_dir_all(".hats").unwrap();
        std::fs::write(".hats/telegram-state.json", "not-json").unwrap();

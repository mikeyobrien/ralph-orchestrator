...
// ABOUTME: Provides the `hats web` command that runs backend and frontend dev servers in parallel.
        ...
        if cleaned == "4.20.0" {
            anyhow::bail!(
                "tsx 4.20.0 has known issues that affect the web server.\n\
                 Fix: npm install tsx@^4.21.0 -w @hats-web/server"
            );
        }
    }
...
pub async fn execute(args: WebArgs) -> Result<()> {
    println!("Starting Hats web servers...");
    ...
    let backend_dir = workspace_root.join("backend/hats-web-server");
    let frontend_dir = workspace_root.join("frontend/hats-web");
    ...
    // Pass HATS_WORKSPACE_ROOT so the backend knows where to spawn hats run from
    // Pass PORT so the backend listens on the configured port
    let mut backend = AsyncCommand::new("npm")
        .args(["run", "dev"])
        .current_dir(&backend_dir)
        .env("HATS_WORKSPACE_ROOT", &workspace_root)
    ...
    // Pass --port for Vite and HATS_BACKEND_PORT for proxy config
    let mut frontend = AsyncCommand::new("npm")
        ...
        .env("HATS_BACKEND_PORT", args.backend_port.to_string())
        .stdout(std::process::Stdio::piped())
        .stderr(std::process::Stdio::piped())
        .spawn()
        .map_err(|e| {

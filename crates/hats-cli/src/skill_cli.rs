//! CLI commands for the `hats tools skill` namespace.
//!
...
use hats_core::{HatsConfig, SkillRegistry};
use serde::Serialize;
...
fn execute_load(root: &Path, name: &str) -> Result<()> {
    ...
    match registry.load_skill(name) {
        ...
        None => {
            ...
            names.sort();
            if names.is_empty() {
                eprintln!("No skills discovered. Check skills.dirs in hats.yml or use --root.");
            } else {
...
fn format_source(skill: &hats_core::SkillEntry) -> String {
    match &skill.source {
        hats_core::SkillSource::BuiltIn => "built-in".to_string(),
        hats_core::SkillSource::File(path) => path.display().to_string(),
    }
}
...
impl From<&hats_core::SkillEntry> for SkillListItem {
    fn from(skill: &hats_core::SkillEntry) -> Self {
        let (source, path) = match &skill.source {
            hats_core::SkillSource::BuiltIn => ("built-in".to_string(), None),
            hats_core::SkillSource::File(path) => {
...
fn find_workspace_root(start: &Path) -> Option<PathBuf> {
    ...
    while let Some(dir) = current {
        if dir.join("hats.yml").exists() || dir.join("hats.yaml").exists() {
...
fn find_default_skills_dir(root: &Path) -> Option<PathBuf> {
    ...
    // Fallback: if the workspace root is nested (hats.yml inside a subdir),
...
fn load_config(root: &Path) -> HatsConfig {
    // Try standard config file names
    let candidates = ["hats.yml", "hats.yaml"];
    let mut config = None;
    for candidate in &candidates {
        let path = root.join(candidate);
        if path.exists()
            && let Ok(loaded) = HatsConfig::from_file(&path)
        {
            config = Some(loaded);
            break;
        }

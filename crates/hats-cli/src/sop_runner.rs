...
//!
//! This module provides functionality for the `hats plan` and `hats code-task` commands,
//! which are thin wrappers that bypass Hats's event loop entirely. They:
...
use hats_adapters::{CliBackend, CustomBackendError, NoBackendError, detect_backend_default};
use hats_core::HatsConfig;
...
///
/// Note: SOPs are copied into crates/hats-cli/sops/ for crates.io packaging.
...
///
/// This is the main entry point for `hats plan` and `hats code-task` commands.
    ...
    let cli_backend = if backend_name == "custom" {
        if let Some(args) = &config.custom_args {
            ...
            CliBackend {
                command,
                args: cli_args,
                prompt_mode: hats_adapters::PromptMode::Arg,
                prompt_flag: None, // Prompt appended as last arg by default
                output_format: hats_adapters::OutputFormat::Text,
            }
        } else {
            ...
            let config_path = config
                .config_path
                .as_deref()
                .unwrap_or_else(|| Path::new("hats.yml"));

            if config_path.exists() {
                let hats_config = HatsConfig::from_file(config_path).map_err(|e| {
                    SopRunError::SpawnError(std::io::Error::new(
                        std::io::ErrorKind::Other,
                        e.to_string(),
                    ))
                })?;
                CliBackend::custom(&hats_config.cli)?
            } else {
                return Err(SopRunError::UnknownBackend(
...
fn resolve_backend(
    ...
    if let Some(path) = config_path
        && path.exists()
        && let Ok(config) = HatsConfig::from_file(path)

//! # hats-cli
//!
//! Binary entry point for the Hats.
//!
//! This crate provides:
//! - CLI argument parsing using `clap`
//! - Application initialization and configuration
//! - Entry point to the headless orchestration loop
//! - Event history viewing via `hats events`
//! - Project initialization via `hats init`
//! - SOP-based planning via `hats plan`
//! - Code task generation via `hats code-task`
//! - Work item tracking via `hats task`
...
use hats_adapters::detect_backend;
use hats_core::{
    CheckStatus, EventHistory, LockError, LoopContext, LoopEntry, LoopLock, LoopRegistry,
    PreflightReport, PreflightRunner, HatsConfig, TerminationReason,
...
impl Verbosity {
    ...
    /// 2. Environment variables: `HATS_VERBOSE=1` or `HATS_QUIET=1`
...
pub enum ConfigSource {
    ...
    Remote(String),
    /// Config override (e.g., "core.scratchpad=.hats/feature/scratchpad.md")
...
pub(crate) fn ensure_scratchpad_directory(config: &HatsConfig) -> anyhow::Result<()> {
...
pub(crate) fn load_config_with_overrides(
    ...
    let mut config = if let Some(ConfigSource::File(path)) = primary_sources.first() {
        ...
        // Only overrides specified - load default hats.yml as base
        let default_path = PathBuf::from("hats.yml");
...
/// Hats - Multi-agent orchestration framework
#[derive(Parser, Debug)]
#[command(name = "hats", version, about)]
struct Cli {
    #[command(subcommand)]
    ...
    #[arg(short, long, default_value = "hats.yml", global = true, action = ArgAction::Append)]
    config: Vec<String>,
...
enum Commands {
    ...
    /// DEPRECATED: Use `hats run --continue` instead.
    ...
    /// Initialize a new hats.yml configuration file
    Init(InitArgs),

    /// Clean up Hats artifacts (.agent/ directory)
    Clean(CleanArgs),
    ...
    /// Hats's runtime tools (agent-facing)
...
struct InitArgs {
    ...
    /// Copy embedded preset to hats.yml
    #[arg(long, conflicts_with = "list_presets")]
    ...
    /// Overwrite existing hats.yml if present
    #[arg(long)]
    force: bool,
}
...
/// user can run `hats resume` to restart reading existing scratchpad."
#[derive(Parser, Debug)]
struct ResumeArgs {
...
struct EmitArgs {
    ...
    /// Path to events file (defaults to .hats/events.jsonl)
    #[arg(long, default_value = ".hats/events.jsonl")]
    pub file: PathBuf,
}
...
/// PDD SOP, bypassing Hats's event loop entirely.
#[derive(Parser, Debug)]
struct PlanArgs {
...
/// This is a thin wrapper that spawns the AI backend with the bundled
/// code-task-generator SOP, bypassing Hats's event loop entirely.
#[derive(Parser, Debug)]
struct CodeTaskArgs {
...
async fn main() -> Result<()> {
    ...
    if tui_enabled {
        ...
        if let Ok((file, _log_path)) =
            hats_core::diagnostics::create_log_file(std::path::Path::new("."))
        {
            if diagnostics_enabled {
                use hats_core::diagnostics::DiagnosticTraceLayer;
                use tracing_subscriber::prelude::*;

                if let Ok(collector) =
                    hats_core::diagnostics::DiagnosticsCollector::new(std::path::Path::new("."))
...
async fn run_command(
    ...
    let mut config = if let Some(source) = primary_sources.first() {
        match source {
            ...
            ConfigSource::Builtin(name) => {
                let preset = presets::get_preset(name).ok_or_else(|| {
                    ...
                    anyhow::anyhow!(
                        "Unknown preset '{}'. Run `hats run --list-presets` to see available presets.\n\nAvailable: {}",
                        name,
                        available
                    )
                })?;
                HatsConfig::parse_yaml(preset.content)
        ...
        if default_path.exists() {
            ...
            warn!("Config file hats.yml not found, using defaults");
            HatsConfig::default()
        }
    };
        ...
        if !scratchpad_path.exists() {
            anyhow::bail!(
                "Cannot continue: scratchpad not found at '{}'. \
                 Start a fresh run with `hats run`.",
    ...
    let (loop_context, _lock_guard) = match LoopLock::try_acquire(workspace_root, &prompt_summary) {
        ...
        Err(LockError::AlreadyLocked(existing)) => {
            ...
            if args.exclusive {
                ...
                let name_generator =
                    hats_core::LoopNameGenerator::from_config(&config.features.loop_naming);
                let loop_id = name_generator.generate_memorable_unique(|name| {
                    hats_core::worktree_exists(workspace_root, name, &worktree_config)
                });
    ...
    if matches!(reason, TerminationReason::RestartRequested) {
        let restart_path = std::path::Path::new(&workspace_root).join(".hats/restart-requested");
...
///
/// Per spec: "When loop terminates due to safeguard (not completion promise),
/// user can run `hats run --continue` to restart reading existing scratchpad,
...
async fn resume_command(
    ...
    eprintln!(
        "{}warning:{} `hats resume` is deprecated. Use `hats run --continue` instead.",
        colors::YELLOW,
        colors::RESET
    );
...
fn init_command(color_mode: ColorMode, args: InitArgs) -> Result<()> {
        ...
        match init::init_from_preset(&preset, backend_override, args.force) {
            Ok(()) => {
                let msg = if let Some(backend) = backend_override {
                    format!(
                        "Created hats.yml from '{}' preset with {} backend",
                        preset, backend
                    )
                } else {
                    format!("Created hats.yml from '{}' preset", preset)
                };
                if use_colors {
        ...
        match init::init_from_backend(&backend, args.force) {
            Ok(()) => {
                if use_colors {
                    ...
                    println!("Created hats.yml with {} backend", backend);
                    println!(
                        "\nNext steps:\n  1. Create PROMPT.md with your task\n  2. Run: hats run"
                    );
                }
    ...
    println!("Initialize a new hats.yml configuration file.\n");
    println!("Usage:");
    println!("  hats init --backend <backend>   Generate minimal config for backend");
    println!("  hats init --preset <preset>     Use an embedded preset");
    println!("  hats init --list-presets        Show available presets\n");
    println!("Backends: claude, kiro, gemini, codex, amp, custom");
    println!("\nRun 'hats init --list-presets' to see available presets.");
...
fn events_command(color_mode: ColorMode, args: EventsArgs) -> Result<()> {
    ...
    // This ensures `hats events` reads from the same events file as the active run
    let history = match args.file {
        Some(path) => EventHistory::new(path),
        None => fs::read_to_string(".hats/current-events")
    ...
    if !history.exists() {
        if use_colors {
            println!(
                "{}No event history found.{} Run `hats` to generate events.",
                colors::DIM,
                colors::RESET
            );
        } else {
            println!("No event history found. Run `hats` to generate events.");
        }
        return Ok(());
    }
...
fn clean_command(
    ...
    if args.diagnostics {
        ...
        return hats_cli::clean_diagnostics(&workspace_root, use_colors, args.dry_run);
    }
...
///
/// Events are written to the path specified in `.hats/current-events` marker file
/// (created by `hats run`), or falls back to `.hats/events.jsonl` if no marker exists.
fn emit_command(color_mode: ColorMode, args: EmitArgs) -> Result<()> {
    ...
    // This ensures `hats emit` writes to the same events file as the active run
    let events_file = fs::read_to_string(".hats/current-events")
...
const TUTORIAL_STEPS: &[TutorialStep] = &[
    TutorialStep {
        ...
        body: &[
            ...
            "Inspect hats with: hats hats list",
            "Visualize the flow with: hats hats graph --format ascii",
        ],
    },
    TutorialStep {
        ...
        body: &[
            ...
            "List built-ins with: hats init --list-presets",
            "Create a config: hats init --preset <name>",
            "Run directly: hats run -c builtin:<name>",
        ],
    },
    TutorialStep {
        ...
        body: &[
            ...
            "Run: hats run -P PROMPT.md or hats run -p \"...\"",
            "Hats emits task.start, hats process events, and the loop ends on done events.",
            "Artifacts live in .hats/agent (scratchpad, tasks, memories).",
            "Check open tasks with: hats tools task ready",
        ],
    },
];
...
fn print_tutorial_outro(use_colors: bool) {
    if use_colors {
        println!(
            "{}Tutorial complete. Next: hats init --list-presets, then hats run.{}",
            colors::GREEN,
            colors::RESET
        );
    } else {
        println!("Tutorial complete. Next: hats init --list-presets, then hats run.");
    }
}

/// Starts a Prompt-Driven Development planning session.
///
/// This is a thin wrapper that bypasses Hats's event loop entirely.
...
mod tests {
    ...
    fn test_config_source_parse_file() {
        let source = ConfigSource::parse("hats.yml");
        match source {
            ConfigSource::File(path) => assert_eq!(path, std::path::PathBuf::from("hats.yml")),
    ...
    fn test_config_source_parse_override_scratchpad() {
        let source = ConfigSource::parse("core.scratchpad=.hats/feature/scratchpad.md");
        match source {
            ConfigSource::Override { key, value } => {
                assert_eq!(key, "core.scratchpad");
                assert_eq!(value, ".hats/feature/scratchpad.md");
            }
    ...
    fn test_bot_daemon_parses_global_config_flag() {
        let cli = Cli::try_parse_from(["hats", "bot", "daemon", "-c", "hats.bot.yml"])
            .expect("CLI parse failed");

        assert!(cli.config.iter().any(|value| value == "hats.bot.yml"));
        assert!(matches!(
            cli.command,
    ...
    fn test_doctor_parses_command() {
        let cli = Cli::try_parse_from(["hats", "doctor"]).expect("CLI parse failed");
    ...
    fn test_tutorial_parses_command() {
        let cli = Cli::try_parse_from(["hats", "tutorial"]).expect("CLI parse failed");
    ...
    fn test_partition_config_sources_separates_overrides() {
        let sources = [
            ConfigSource::File(PathBuf::from("hats.yml")),
            ConfigSource::Override {
                key: "core.scratchpad".to_string(),
...
",
        ...
        let mut config = HatsConfig::from_file(&config_path).unwrap();
    ...
    fn test_format_preflight_summary_with_failures() {
        let report = PreflightReport {
            ...
            failures: 1,
            checks: vec![
                hats_core::CheckResult::pass("config", "Config"),
                hats_core::CheckResult::warn("backend", "Backend", "Missing"),
                hats_core::CheckResult::fail("paths", "Paths", "Missing path"),
            ],
        };
    ...
    fn test_load_config_with_overrides_applies_override_sources() {
        ...
        let config_path = temp_dir.path().join("hats.yml");
    ...
    fn test_load_config_with_overrides_missing_file_falls_back_to_defaults() {
        ...
        let default = HatsConfig::default();

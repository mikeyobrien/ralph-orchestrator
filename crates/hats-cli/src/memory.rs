//! CLI commands for the `hats memory` namespace.
//!
...
use hats_core::{MarkdownMemoryStore, Memory, MemoryType};
...
fn list_command(store: &MarkdownMemoryStore, args: ListArgs, use_colors: bool) -> Result<()> {
    ...
    if memories.is_empty() {
        if use_colors {
            ...
            println!("\nNo memories yet.\n");
            println!("Create your first memory:");
            println!("  hats tools memory add \"<content>\" -t pattern --tags tag1,tag2\n");
            println!("Memory types: pattern, decision, fix, context");
            println!();
        }
        return Ok(());
...
fn search_command(store: &MarkdownMemoryStore, args: SearchArgs, use_colors: bool) -> Result<()> {
    ...
    if memories.is_empty() {
        if use_colors {
            ...
            println!(
                "{}Try a different search term or use `hats tools memory list` to see all.{}\n",
                colors::DIM,
                colors::RESET
            );
        } else {
            ...
            println!("Try a different search term or use `hats tools memory list` to see all.\n");
        }
        return Ok(());
    }
...
fn print_memories_table(memories: &[Memory], use_colors: bool) {
    ...
    if use_colors {
        println!(
            "\n{DIM}Showing {} memories • Use `hats tools memory show <id>` for details{RESET}",
            memories.len()
        );
    } else {
        println!(
            "\nShowing {} memories • Use `hats tools memory show <id>` for details",
            memories.len()
        );
    }
}

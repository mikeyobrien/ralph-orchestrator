//! CLI commands for the `hats loops` namespace.
//!
//! Manage parallel Hats loops running in git worktrees.
//!
//! Subcommands:
...
use hats_core::worktree::{list_hats_worktrees, remove_worktree};
use hats_core::{LoopRegistry, MergeButtonState, MergeQueue, MergeState, merge_button_state};
...
#[derive(Parser, Debug)]
pub struct LogsArgs {
    /// Loop ID (e.g., hats-20250124-a3f2 or just a3f2)
    pub loop_id: String,
...
fn list_loops(args: ListArgs, use_colors: bool) -> Result<()> {
    ...
    let worktrees = list_hats_worktrees(&cwd).unwrap_or_default();
    ...
    for wt in &worktrees {
        if wt.branch.starts_with("hats/") {
            let loop_id = wt.branch.trim_start_matches("hats/");
    ...
    if has_needs_review {
        println!("Hint: Use `hats loops retry <id>` to retry failed merges.");
    }
    println!("Use `hats loops --help` for more commands.");
...
fn show_logs(args: LogsArgs) -> Result<()> {
    ...
    let events_path = base_path.join(".hats/events.jsonl");

    if !events_path.exists() {
        // Fallback: show history file instead
        let history_path = base_path.join(".hats/history.jsonl");
...
fn show_history(args: HistoryArgs) -> Result<()> {
    ...
    let history_path = if let Some(wt_path) = worktree_path {
        PathBuf::from(wt_path).join(".hats/history.jsonl")
    } else {
        cwd.join(".hats/history.jsonl")
    };
...
fn retry_merge(args: RetryArgs) -> Result<()> {
    ...
    spawn_merge_hats(&cwd, &args.loop_id)
}
...
fn stop_loop(args: StopArgs) -> Result<()> {
    ...
    let stop_path = target_root.join(".hats/stop-requested");
    if let Some(parent) = stop_path.parent() {
        std::fs::create_dir_all(parent)
            .context("Failed to create .hats directory for stop signal")?;
    }
...
fn prune_stale() -> Result<()> {
    ...
    if orphan_count > 0 {
        println!("\nTo remove orphan worktrees, use: hats loops discard <id>");
    }
...
fn show_diff(args: DiffArgs) -> Result<()> {
    ...
    // Find the branch
    let branch = format!("hats/{}", loop_id);
...
fn merge_loop(args: MergeArgs) -> Result<()> {
    ...
    if let Ok(Some(entry)) = merge_queue.get_entry(&loop_id) {
        ...
        let is_orphan = worktrees
            .iter()
            .any(|wt| wt.branch == format!("hats/{}", loop_id));
    ...
    spawn_merge_hats(&cwd, &loop_id)
}

/// Helper to spawn merge-hats
fn spawn_merge_hats(cwd: &std::path::Path, loop_id: &str) -> Result<()> {
    ...
    let config_path = cwd.join(".hats/merge-loop-config.yml");
    std::fs::write(&config_path, preset.content).context("Failed to write merge config file")?;

    // Spawn merge-hats
    println!("Spawning merge-hats for loop '{}'...", loop_id);

    let status = Command::new("hats")
        .args([
            "run",
            "-c",
        ...
        .status()
        .context("Failed to spawn merge-hats")?;

    if !status.success() {
        bail!("merge-hats exited with error");
    }
...
fn resolve_loop(cwd: &std::path::Path, id: &str) -> Result<(String, Option<String>)> {
    ...
    // Try partial match (e.g., "a3f2" matches "hats-20250124-143052-a3f2")
    ...
    for wt in worktrees {
        if wt.branch.ends_with(id) || wt.branch.contains(id) {
            let loop_id = wt.branch.trim_start_matches("hats/").to_string();
...
mod tests {
    ...
    use hats_core::LoopLock;
    use hats_core::loop_registry::LoopEntry;
    use std::process::Command;
    ...
    fn test_shorten_path() {
        ...
        assert_eq!(shorten_path("./worktrees/hats-abc"), "hats-abc");
    }
    ...
    fn test_show_logs_falls_back_to_history() {
        ...
        std::fs::create_dir_all(".hats").expect("create .hats");
        std::fs::write(
            ".hats/history.jsonl",
    ...
    fn test_stop_loop_writes_stop_requested_file() {
        ...
        assert!(temp_dir.path().join(".hats/stop-requested").exists());
    }
    ...
    fn test_show_diff_missing_branch_errors() {
        ...
        assert!(
            err.to_string()
                .contains("Branch 'hats/loop-missing-branch' not found")
        );
    }

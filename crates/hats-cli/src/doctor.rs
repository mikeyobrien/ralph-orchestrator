//! CLI command for `hats doctor`.

use anyhow::Result;
use clap::Parser;
use hats_adapters::{CliBackend, DEFAULT_PRIORITY};
use hats_core::{CheckResult, CheckStatus, ConfigError, HatBackend, PreflightReport, HatsConfig};
...
pub async fn execute(
    ...
    let runner = hats_core::PreflightRunner::default_checks();
...
fn hat_collection_check(_config: &HatsConfig) -> CheckResult {
    let config = _config;
...
fn auth_backend_names(config: &HatsConfig) -> Vec<String> {

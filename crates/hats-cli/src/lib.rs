...
/// Clean diagnostic logs from .hats/diagnostics directory
pub fn clean_diagnostics(workspace_root: &Path, use_colors: bool, dry_run: bool) -> Result<()> {
    let diagnostics_dir = workspace_root.join(".hats/diagnostics");
...
mod tests {
    ...
    fn clean_diagnostics_no_dir_is_ok() {
        ...
        assert!(result.is_ok());
        assert!(!temp_dir.path().join(".hats/diagnostics").exists());
    }

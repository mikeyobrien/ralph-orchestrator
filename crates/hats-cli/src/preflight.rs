...
use hats_core::{CheckResult, CheckStatus, PreflightReport, PreflightRunner, HatsConfig};
...
pub(crate) fn config_source_label(config_sources: &[ConfigSource]) -> String {
    ...
    match primary {
        ...
        Some(ConfigSource::Override { .. }) | None => "hats.yml".to_string(),
    }
}
...
mod tests {
    ...
    fn config_source_label_handles_sources() {
        let file_label =
            config_source_label(&[ConfigSource::File(PathBuf::from("/tmp/hats.yml"))]);
        assert_eq!(file_label, "/tmp/hats.yml");

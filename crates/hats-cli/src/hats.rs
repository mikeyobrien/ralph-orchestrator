//! CLI commands for the `hats hats` namespace.
//!
...
use hats_adapters::{CliBackend, detect_backend_default};
use hats_core::{HatRegistry, HatsConfig};
...
fn load_config(config_sources: &[ConfigSource]) -> Result<HatsConfig> {
    ...
    match source {
        ...
        ConfigSource::Builtin(name) => {
            let preset = presets::get_preset(name).ok_or_else(|| {
                ...
                anyhow::anyhow!(
                    "Unknown preset '{}'. Run `hats init --list-presets` to see available presets.\n\nAvailable: {}",
                    name,
                    available
                )
            })?;
        ...
        ConfigSource::Override { key, value } => {
            ...
            Err(anyhow::anyhow!(
                "Config overrides are not supported for `hats hats`.\n\nPlease use a local config file or builtin preset instead.\nOverride: {}={}",
                key,
                value
            ))
        }
...
fn list_hats<W: Write>(writer: &mut W, registry: &HatRegistry, _use_colors: bool) -> Result<()> {
    if registry.is_empty() {
        writeln!(
            writer,
            "No custom hats configured (using default HatlessHats coordination)."
        )?;
        return Ok(());
    }
...
fn validate_hats<W: Write>(
    ...
    if let Some(start) = &config.event_loop.starting_event {
        ...
        writeln!(writer, "Entry: task.start (Hats coordinates)")?;
    }
    writeln!(writer)?;
    ...
    for hat in registry.all() {
        for pub_event in &hat.publishes {
            ...
            // Ignore if Hats subscribes (task.start, etc - though Hats usually PUBLISHES task.start)
            // Hats conceptually subscribes to everything as fallback, but we want to warn if no SPECIFIC hat handles it.
...
///
/// Shows the logical flow: task.start -> Hats -> Hats
...
/// 1. CLI flag (`--backend`)
/// 2. Config file (`cli.backend` in hats.yml)
/// 3. Auto-detect (first available from claude → kiro → gemini → codex → amp)
fn resolve_backend(flag_override: Option<&str>, config: &HatsConfig) -> Result<String> {
...
fn build_diagram_prompt(registry: &HatRegistry) -> String {
    ...
    prompt.push_str("- task.start → Hats\n");

    // Collect all hats sorted for deterministic output
    let mut hats: Vec<_> = registry.all().collect();
    hats.sort_by(|a, b| a.name.cmp(&b.name));

    // Hats -> Hats (based on subscriptions)
    for hat in &hats {
...
fn generate_mermaid_string(registry: &HatRegistry) -> String {
    ...
    output.push_str("    Start[task.start] --> Hats\n");

    // Reconstruct Hats's publishes (what hats subscribe to)
    let mut hats_publishes: HashSet<String> = HashSet::new();
    for hat in registry.all() {
        for sub in &hat.subscriptions {
            hats_publishes.insert(sub.as_str().to_string());
        }
    }
    ...
    // Even though everything goes through Hats, it's useful to see A -> B
...
mod tests {
    ...
    fn test_graph_hats_mermaid() {
        ...
        assert!(output.contains("Hats -->|start| A"));
        assert!(output.contains("A -->|mid| Hats"));
        assert!(output.contains("Hats -->|mid| B"));
    }
    ...
    fn test_graph_hats_ascii() {
        ...
        assert!(output.contains("Builder") || output.contains("Hats"));
    }
    ...
    fn test_graph_hats_unicode() {
        ...
        assert!(output.contains("Coder") || output.contains("Hats"));
    }
    ...
    fn test_build_diagram_prompt() {
        ...
        assert!(prompt.contains("task.start → Hats"));
        assert!(prompt.contains("Hats → Builder"));
        assert!(prompt.contains("build.task"));
        assert!(prompt.contains("build.done"));
        assert!(prompt.contains("Hats → Tester"));

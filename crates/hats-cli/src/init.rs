//! Init command implementation for hats.
//!
//! Handles initialization of hats.yml configuration files, either from
...
pub enum InitError {
    ...
    #[error("Failed to write hats.yml: {0}")]
...
fn generate_template(backend: &str) -> String {
    format!(
        r#"# Hats Configuration
# Generated by: hats init --backend {backend}
# Docs: https://github.com/mikeyobrien/hats
...
# Create PROMPT.md with your task, then run: hats run
"#
    )
}

/// Checks if hats.yml exists and handles the force flag.
fn check_file_exists(force: bool) -> Result<(), InitError> {
    let path = Path::new("hats.yml");
...
/// Initializes hats.yml from a minimal backend template.
///
/// # Arguments
/// * `backend` - The backend name (claude, kiro, gemini, codex, amp, copilot, opencode, custom)
/// * `force` - If true, overwrite existing hats.yml
///
/// # Errors
...
pub fn init_from_backend(backend: &str, force: bool) -> Result<(), InitError> {
    ...
    fs::write("hats.yml", content)?;

    Ok(())
}

/// Initializes hats.yml from an embedded preset.
///
/// # Arguments
...
pub fn format_preset_list() -> String {
    ...
    output.push_str("\nUsage: hats init --preset <preset-name>\n");
    output
}
...
mod tests {
    ...
    fn test_init_from_preset_code_assist_writes_config() {
        ...
        let content = fs::read_to_string("hats.yml").expect("read hats.yml");
        assert!(

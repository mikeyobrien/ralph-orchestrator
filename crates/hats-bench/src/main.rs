//! # hats-bench
//!
//! Benchmark harness for the Hats.
//!
//! This crate provides:
...
use hats_adapters::{CliBackend, CliExecutor, detect_backend};
use hats_core::{
    CleanupPolicy, CliCapture, EventLoop, PlayerConfig, HatsConfig, ReplayMode, SessionPlayer,
...
/// Hats Benchmark Harness - Record, replay, and benchmark orchestration loops
#[derive(Parser, Debug)]
#[command(name = "hats-bench", version, about)]
struct Args {
    /// Verbose output
    #[arg(short, long)]
    verbose: bool,
...
async fn run_task_loop(
    task: &hats_core::TaskDefinition,
    workspace: &hats_core::TaskWorkspace,
    record_path: Option<&PathBuf>,
    record_ux: bool,
) -> Result<(u32, String)> {
    use hats_core::{Record, SessionRecorder};
    use std::sync::Arc;

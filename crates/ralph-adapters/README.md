...
| Process exit semantics | `autoloop_runner` preserves the documented success/non-zero exit status but never surfaces raw child stdout/stderr: a non-zero exit yields only the executable/action, exit code, and a generic bounded explanation. The CLI maps that result and terminal events into Ralph termination reasons. |

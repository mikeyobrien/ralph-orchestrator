# Task: Implement Native Dynamic SOP Integration

## Goal Description
Implement "Native Dynamic SOP Integration" using the off-the-shelf `code-assist.sop.md` from the `strands-agents/agent-sop` repository. This minimizes custom changes while enabling the agent to follow a standardized, "native" workflow. We also configure `PROMPT.md` to teach the agent how to discover, load, and execute these SOPs.

## Implementation Status: COMPLETE

### Summary
Implemented the Native Dynamic SOP Integration with the following components:

1. **SOP Registry Directory**: Created `.agent/sops/` directory structure
2. **Code Assist SOP**: Created `.agent/sops/code-assist.sop.md` with full TDD workflow
3. **`ralph sop` Subcommand**: Implemented in `src/ralph_orchestrator/__main__.py`
   - Accepts standard run arguments (agent, config, max-iterations, etc.)
   - Reads prompt from file or direct text
   - Prepends the Native Execution Protocol to the prompt
   - Falls through to standard orchestrator execution
4. **Unit Tests**: Created `tests/test_sop_command.py` with 12 tests covering:
   - Subcommand parsing
   - Protocol constant definition
   - Prompt generation (from text and file)
   - Command execution flow
   - Init directory integration
   - SOP file existence and structure

### Test Results
- All 12 SOP-specific tests pass
- Full test suite: 1010 passed, 34 skipped (integration tests requiring API keys)
...
## Proposed Changes
### SOP Registry
#### [NEW] [.agent/sops/code-assist.sop.md](file:///home/mobrienv/code/ralph-orchestrator/.agent/sops/code-assist.sop.md)
- Create the directory `.agent/sops`
- Create the file `code-assist.sop.md` with the exact content from `strands-agents/agent-sop` (v0.1.0 equivalent):
...
- **mode** (optional, default: "auto"): The interaction mode.
  - "auto": No user interaction after initial setup. (**Note**: "interactive" mode is NOT supported in Ralph).

**Constraints for parameter acquisition:**
- You MUST ask for all parameters upfront in a single prompt.
- You MUST enforce `mode="auto"` as Ralph runs in a non-interactive loop.
...
### CLI Command
#### [MODIFY] [src/ralph_orchestrator/__main__.py](file:///home/mobrienv/code/ralph-orchestrator/src/ralph_orchestrator/__main__.py)
- Implement `ralph sop` subcommand.
- This command will:
    1.  Accept standard run arguments (agent, config, etc.).
    2.  Read the current `PROMPT.md` (or specified file).
    3.  Prepend the **Native Execution Protocol** (defined below) to the prompt text.
    4.  Initialize `RalphConfig` with `prompt_text` set to this combined content.
    5.  Execute the orchestrator.

**Native Execution Protocol (injected at runtime):**
```markdown
...
1.  **Setup Test Environment**:
    *   Create a temporary directory: `mkdir -p /tmp/ralph_sop_test`
    *   Copy the `ralph` source code to the temp dir (or install in editable mode).
    *   Initialize a new project: `cd /tmp/ralph_sop_test && ralph init`
    *   **Simulate SOP Registry**: Copy the `.agent/sops` directory from the main repo to `/tmp/ralph_sop_test/.agent/sops`.

2.  **Create Test SOP**:
    *   Create `/tmp/ralph_sop_test/.agent/sops/hello-world.sop.md`:
        ```markdown
        # Hello World SOP
        ## Steps
        1. Create a greeting file.
           Constraints:
           - You MUST Run: `echo "Hello form SOP" > greeting.txt`
        ```

3.  **Execute Live Test (Claude/Gemini)**:
    *   Run: `ralph sop --prompt-text "Run the hello-world SOP" --agent claude --max-iterations 5`
    *   **Validation**: Assert that `greeting.txt` exists and contains "Hello form SOP".

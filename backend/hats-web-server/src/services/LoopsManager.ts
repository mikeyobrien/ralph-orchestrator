 ...
 * LoopsManager
 *
 * Manages hats loops for parallel task execution.
 ...
 * - Periodically run `hats loops process` to clear merge queue
...
export interface LoopsManagerOptions {
  ...
  /** Path to hats executable (default: "hats") */
  hatsPath?: string;
  /** Workspace root directory (cwd for hats subprocesses) */
  workspaceRoot?: string;
}

/**
 * Manages hats loops for parallel execution.
...
export class LoopsManager extends EventEmitter {
  ...
  async listLoops(): Promise<LoopStatus[]> {
    const output = await this.runHatsCommand(["loops", "list", "--json"]);
    try {
      const loops = JSON.parse(output);
      return loops;
    } catch {
      // If JSON parsing fails, return empty array
      // (hats loops list --json may not be available in all versions)
      return [];
    }
  }
  ...
  async pruneStale(): Promise<void> {
    try {
      await this.runHatsCommand(["loops", "prune"]);
   ...
   * If steeringInput is provided, it's written to .hats/merge-steering.txt
   * for the merge-hats process to read and incorporate into its strategy.
   */
  async retryMerge(loopId: string, steeringInput?: string): Promise<void> {
    // Write steering input to file for merge-hats to read
    if (steeringInput?.trim()) {
      const steeringPath = `${process.cwd()}/.hats/merge-steering.txt`;
      const fs = await import("fs/promises");
      await fs.writeFile(steeringPath, steeringInput.trim(), "utf-8");
    }

    await this.runHatsCommand(["loops", "retry", loopId]);
  }
  ...
  async discardLoop(loopId: string): Promise<void> {
    ...
    await this.runHatsCommand(["loops", "discard", "-y", loopId]);
  }
  ...
  async stopLoop(loopId: string, force?: boolean): Promise<void> {
    ...
    await this.runHatsCommand(args);
  }
  ...
  async getMergeButtonState(loopId: string): Promise<MergeButtonState> {
    const output = await this.runHatsCommand(["loops", "merge-button-state", loopId]);
  ...
  /**
   * Run a hats command and return stdout
   */
  private runHatsCommand(args: string[]): Promise<string> {
    return new Promise((resolve, reject) => {
      const proc = spawn(this.hatsPath, args, {
      ...
      proc.on("close", (code) => {
        if (code === 0) {
          ...
          reject(new Error(`hats ${args.join(" ")} failed (exit ${code}): ${stderr}`));
        }
      });

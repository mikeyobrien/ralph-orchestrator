...
/**
 * Result from HatsRunner (partial interface for what we need)
 */
...
export interface TaskBridgeOptions {
  ...
  defaultCwd: string;
  /** Task type to use for queue (default: 'hats.run') */
  taskType?: string;
...
export class TaskBridge {
  ...
  constructor(
    ...
    this.taskType = options.taskType ?? "hats.run";
      ...
      if (this.configMerger && this.defaultConfigPath) {
        ...
        if (preset) {
          ...
          if (builtinMatch) {
            ...
            const presetPath = path.join(this.defaultCwd, ".hats", "hats", `${presetName}.yml`);
            args.push("-c", presetPath);
            configResolved = true;
          } else if (this.collectionService) {
            const yamlContent = this.collectionService.exportToYaml(preset);
            if (yamlContent) {
              const tempDir = path.join(this.defaultCwd, ".hats", "temp");
  ...
  /**
   * Reconnect to running hats processes after server restart.
   ...
   * Checks both `.hats/loops.json` (worktree loops) and `.hats/loop.lock` (primary loop).
   *
  ...
  private resolveLoopId(taskTitle: string): string | null {
    try {
      ...
      const loopsPath = path.join(repoRoot, ".hats", "loops.json");
      ...
      const lockPath = path.join(repoRoot, ".hats", "loop.lock");
   ...
   * The loop entry in `.hats/loops.json` may appear with a slight delay

...
describe("enqueueTask preset handling", () => {
  ...
  test("directory preset resolves to .hats/hats/ file path", async () => {
    ...
    const expectedPath = path.join(defaultCwd, ".hats", "hats", "my-preset.yml");
    assert.deepStrictEqual(
      payload.args,
      ["-c", expectedPath],
  ...
  test("collection preset (UUID) exports to temp file and uses that path", async () => {
    ...
    // Verify the path is a temp file in .hats/temp/
    const configPath = payload.args[1];
    assert.ok(
      configPath.includes(".hats/temp/collection-"),
      `Config path should be in .hats/temp/: ${configPath}`
    );

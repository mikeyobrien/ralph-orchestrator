...
const testRunDir = path.join(os.tmpdir(), "hats-test-runs");
...
test("ProcessSupervisor.spawn treats shell metacharacters as literals (CVE regression)", () => {
  ...
  // "INJECTED" to stdout. With array-form spawn, hats sees "; echo INJECTED" as
  ...
  while (Date.now() - start < maxWait) {
    ...
    if (fs.existsSync(stderrPath)) {
      ...
      // If hats received the malicious args as literal arguments, it will complain

 ...
 * ProcessSupervisor
 *
 * Manages detached hats processes that can survive server restarts.
 * Handles process spawning, reconnection, and state tracking via filesystem.
 *
 * Directory structure:
 *   ~/.hats/web/runs/{taskId}/
 *     - pid: Process ID
...
export class ProcessSupervisor {
  ...
  constructor(options?: ProcessSupervisorOptions) {
    this.runDir = options?.runDir ?? path.join(os.homedir(), ".hats/web/runs");
  }

  /**
   * Spawn a detached hats process
   */
  spawn(taskId: string, prompt: string, args: string[], cwd: string, command: string = "hats"): ProcessHandle {

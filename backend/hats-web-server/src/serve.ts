...
import { createHatsTaskHandler } from "./runner/HatsTaskHandler";
...
// 1. HATS_WORKSPACE_ROOT env var (explicit override)
// 2. process.cwd() (where user launched the server)
// 3. Fallback to repo root (computed from script location)
const REPO_ROOT = path.resolve(__dirname, "../../..");
const CWD = process.env.HATS_WORKSPACE_ROOT || process.cwd() || REPO_ROOT;
...
// HATS_MAX_CONCURRENT: Max parallel tasks (default: 3, range: 1-10)
const MAX_CONCURRENT = parseInt(process.env.HATS_MAX_CONCURRENT || "3", 10);
const maxConcurrent = Math.max(1, Math.min(MAX_CONCURRENT, 10)); // Cap at 10

// HATS_POLL_INTERVAL_MS: Dispatcher poll interval (default: 100ms)
const pollIntervalMs = parseInt(process.env.HATS_POLL_INTERVAL_MS || "100", 10);

// HATS_TASK_TIMEOUT_MS: Task timeout (default: 14400000ms = 4 hours)
const taskTimeoutMs = parseInt(process.env.HATS_TASK_TIMEOUT_MS || "14400000", 10);

// HATS_LOOPS_PROCESS_INTERVAL_MS: Merge queue processing interval (default: 30000ms = 30s)
const loopsProcessIntervalMs = parseInt(process.env.HATS_LOOPS_PROCESS_INTERVAL_MS || "30000", 10);
...
const isTestMode = process.env.HATS_TEST_MODE === "1";

// Default config path for hats runs (can be overridden by user preset)
const defaultConfigPath = process.env.HATS_CONFIG_PATH ?? path.resolve(REPO_ROOT, "hats.yml");

if (isTestMode) {
  dispatcher.registerHandler("test.log", createTestLogTaskHandler());
} else {
  // Register the hats task handler
  ...
  console.log(`Default hats config: ${defaultConfigPath}`);
  dispatcher.registerHandler(
    "hats.run",
    createHatsTaskHandler({
      defaultCwd: CWD,
...
startServer({ port: PORT, host: HOST, db, taskBridge, loopsManager, planningService })
  .then(() => {
    ...
    // Reconnect to running hats processes (Phase 5)

...
describe("task schema supports new fields for UX improvements", () => {
  ...
  it("loopId field can be stored and retrieved", () => {
    const task = taskRepository.create({
      ...
      status: "running",
      priority: 2,
      loopId: "hats-20260128-041234-abc1",
    });

    assert.equal(task.loopId, "hats-20260128-041234-abc1");

    const retrieved = taskRepository.findById("task-loop-test");
    assert.equal(retrieved?.loopId, "hats-20260128-041234-abc1");
  });

...
/**
 * Loops router - operations for managing hats loops
 */
export const loopsRouter = router({
   ...
   * Steering input provides guidance to the merge-hats process
...
export const collectionRouter = router({
  ...
  /**
   * Export a collection to Hats YAML preset format
   */
...
/**
 * Config router - operations for reading/writing hats.yml configuration
 *
...
const CONFIG_PATH = path.join(REPO_ROOT, "hats.yml");

export const configRouter = router({
  /**
   * Get the current hats.yml configuration
  ...
  get: publicProcedure.query(() => {
    ...
    if (!fs.existsSync(configPath)) {
      throw new TRPCError({
        code: "NOT_FOUND",
        message: "Configuration file not found at hats.yml",
      });
    }
    ...
    try {
      ...
      console.warn("[Config] Failed to parse hats.yml:", err);
    }
  ...
  /**
   * Update the hats.yml configuration
...
export function getDirectoryPresets(): Preset[] {
  const hatsDir = path.resolve(process.cwd(), ".hats/hats");

 ...
 * reading and writing the hats.yml configuration file.
 *
...
const TEST_CONFIG_PATH = path.join(REPO_ROOT, "hats.yml");
...
describe("config.get tRPC endpoint", () => {
  ...
  test("returns valid YAML structure", async () => {
    ...
    // Then: Parsed config should have expected hats.yml fields
    const parsed = result.parsed as Record<string, unknown>;
    // The config should have at least one of these typical hats.yml sections
    const hasValidSection =
      "event_loop" in parsed ||
      "cli" in parsed ||
      "core" in parsed ||
      "hats" in parsed;
    assert.ok(hasValidSection, "Config should have recognized hats.yml sections");
  });
});

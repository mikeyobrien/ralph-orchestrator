...
def write_fixture(workspace: Path, records: list[dict[str, object]] | None = None) -> tuple[Path, Path, Path]:
    # Ralph roots engine state beneath `.ralph/autoloop`. The fake Ralph below
    # writes where a real Ralph-launched engine writes, so the runner is
    # exercised against the same layout the live smoke uses.
    run_dir = workspace / ".ralph/autoloop/runs/fake-six-provider"
    run_dir.mkdir(parents=True)
    journal = workspace / ".ralph/autoloop/journal.jsonl"
    journal.write_text(
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            fake_ralph = executable(
                ...
                textwrap.dedent(
                    ...
                    run_dir = cwd / ".ralph/autoloop/runs" / run
                    run_dir.mkdir(parents=True)
                    evidence = run_dir / "smoke-evidence.txt"
                    ...
                    (cwd / ".ralph/autoloop/journal.jsonl").write_text("".join(json.dumps(r) + "\\n" for r in records))
            ...
            self.assertEqual(
                (retained / ".ralph/autoloop/runs/fake-six-provider/smoke-evidence.txt").read_text().splitlines(),
...
class PromptProbeContract(unittest.TestCase):
    ...
    def test_fixed_probes_use_the_rendered_run_state_directory(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp:
            workspace = Path(temp)
            run_dir = workspace / ".ralph/autoloop/runs/live-smoke"
            run_dir.mkdir(parents=True)
    ...
    def test_probe_fails_closed_when_rendered_run_directory_is_missing(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            workspace = Path(temp)
            missing = workspace / ".ralph/autoloop/runs/missing"
...
class HandoffGateContract(unittest.TestCase):
    def run_gate(self, workspace: Path, iteration: int, records: list[dict[str, object]]) -> subprocess.CompletedProcess[str]:
        ...
        state_dir = workspace / ".ralph/autoloop/runs" / run_id
...
class NativePresetSelectionIntegration(unittest.TestCase):
    # These cases invoke `autoloop run` directly, with no Ralph launch, so the
    # engine keeps its own `.autoloop` default state root. Only the
    # Ralph-launched paths above move to `.ralph/autoloop`.

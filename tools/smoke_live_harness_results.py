...
def field(record: dict[str, Any], name: str, default: Any = None) -> Any:
    """Read a native-journal field. The dataclass helper is imported as
    `dataclass_field` so this name never collides with it."""
...
def validate(records: list[dict[str, Any]], evidence: list[str]) -> tuple[list[Row], list[str], str, str, str, str]:
    ...
    for index, row in enumerate(rows, 1):
        ...
        if len(finishes) <= index - 1:
            ...
            if row.handoff and len(starts) > index - 1 and not (
                records.index(starts[index - 1]) < records.index(matching[0]) < records.index(finish)
            ):

...
# Ralph owns the engine state layout. It launches autoloop with runtime state
# rooted beneath `.ralph/autoloop`, never autoloop's standalone `.autoloop`
# default, so the smoke reads the run back from the same place Ralph wrote it.
ENGINE_STATE_DIR="$WORKSPACE/.ralph/autoloop"
JOURNAL_PATH="$ENGINE_STATE_DIR/journal.jsonl"
EVIDENCE_PATH="$ENGINE_STATE_DIR/missing-smoke-evidence.txt"
run_dirs=("$ENGINE_STATE_DIR"/runs/*)
